from __future__ import annotations
import typing
import aws_cdk
import constructs
import pydantic
import datetime
from ._base import BaseConstruct, BaseClass, BaseStruct, BaseCfnResource, BaseCfnProperty, ConnectableMixin, BaseMethodParams, GenericApplyRemovalPolicyParams, REQUIRED_INIT_PARAM, _REQUIRED_INIT_PARAM

#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.BridgeFlowSourceProperty
class CfnBridge_BridgeFlowSourcePropertyDef(BaseStruct):
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the cloud flow used as a source of this bridge.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the flow source.\n')
    flow_vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The name of the VPC interface attachment to use for this source.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-bridgeflowsource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_flow_source_property = mediaconnect.CfnBridge.BridgeFlowSourceProperty(\n        flow_arn="flowArn",\n        name="name",\n\n        # the properties below are optional\n        flow_vpc_interface_attachment=mediaconnect.CfnBridge.VpcInterfaceAttachmentProperty(\n            vpc_interface_name="vpcInterfaceName"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['flow_arn', 'name', 'flow_vpc_interface_attachment']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.BridgeFlowSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.BridgeNetworkOutputProperty
class CfnBridge_BridgeNetworkOutputPropertyDef(BaseStruct):
    ip_address: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output IP Address.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output name.\n')
    network_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description="The network output's gateway network name.\n")
    port: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output port.\n')
    protocol: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output protocol.\n')
    ttl: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output TTL.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-bridgenetworkoutput.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_network_output_property = mediaconnect.CfnBridge.BridgeNetworkOutputProperty(\n        ip_address="ipAddress",\n        name="name",\n        network_name="networkName",\n        port=123,\n        protocol="protocol",\n        ttl=123\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['ip_address', 'name', 'network_name', 'port', 'protocol', 'ttl']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.BridgeNetworkOutputProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.BridgeNetworkSourceProperty
class CfnBridge_BridgeNetworkSourcePropertyDef(BaseStruct):
    multicast_ip: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network source multicast IP.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the network source. This name is used to reference the source and must be unique among sources in this bridge.\n')
    network_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description="The network source's gateway network name.\n")
    port: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network source port.\n')
    protocol: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network source protocol.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-bridgenetworksource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_network_source_property = mediaconnect.CfnBridge.BridgeNetworkSourceProperty(\n        multicast_ip="multicastIp",\n        name="name",\n        network_name="networkName",\n        port=123,\n        protocol="protocol"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['multicast_ip', 'name', 'network_name', 'port', 'protocol']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.BridgeNetworkSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.BridgeOutputProperty
class CfnBridge_BridgeOutputPropertyDef(BaseStruct):
    network_output: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeNetworkOutputPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The output of the bridge. A network output is delivered to your premises.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-bridgeoutput.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_output_property = mediaconnect.CfnBridge.BridgeOutputProperty(\n        network_output=mediaconnect.CfnBridge.BridgeNetworkOutputProperty(\n            ip_address="ipAddress",\n            name="name",\n            network_name="networkName",\n            port=123,\n            protocol="protocol",\n            ttl=123\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['network_output']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.BridgeOutputProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.BridgeSourceProperty
class CfnBridge_BridgeSourcePropertyDef(BaseStruct):
    flow_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeFlowSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The source of the bridge. A flow source originates in MediaConnect as an existing cloud flow.\n')
    network_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeNetworkSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The source of the bridge. A network source originates at your premises.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-bridgesource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_source_property = mediaconnect.CfnBridge.BridgeSourceProperty(\n        flow_source=mediaconnect.CfnBridge.BridgeFlowSourceProperty(\n            flow_arn="flowArn",\n            name="name",\n\n            # the properties below are optional\n            flow_vpc_interface_attachment=mediaconnect.CfnBridge.VpcInterfaceAttachmentProperty(\n                vpc_interface_name="vpcInterfaceName"\n            )\n        ),\n        network_source=mediaconnect.CfnBridge.BridgeNetworkSourceProperty(\n            multicast_ip="multicastIp",\n            name="name",\n            network_name="networkName",\n            port=123,\n            protocol="protocol"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['flow_source', 'network_source']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.BridgeSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.EgressGatewayBridgeProperty
class CfnBridge_EgressGatewayBridgePropertyDef(BaseStruct):
    max_bitrate: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The maximum expected bitrate (in bps) of the egress bridge.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-egressgatewaybridge.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    egress_gateway_bridge_property = mediaconnect.CfnBridge.EgressGatewayBridgeProperty(\n        max_bitrate=123\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['max_bitrate']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.EgressGatewayBridgeProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.FailoverConfigProperty
class CfnBridge_FailoverConfigPropertyDef(BaseStruct):
    failover_mode: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The type of failover you choose for this flow. MERGE combines the source streams into a single stream, allowing graceful recovery from any single-source loss. FAILOVER allows switching between different streams.\n')
    source_priority: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_SourcePriorityPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The priority you want to assign to a source. You can have a primary stream and a backup stream or two equally prioritized streams. This setting only applies when Failover Mode is set to FAILOVER.\n')
    state: typing.Optional[str] = pydantic.Field(None, description='The state of source failover on the flow. If the state is inactive, the flow can have only one source. If the state is active, the flow can have one or two sources.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-failoverconfig.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    failover_config_property = mediaconnect.CfnBridge.FailoverConfigProperty(\n        failover_mode="failoverMode",\n\n        # the properties below are optional\n        source_priority=mediaconnect.CfnBridge.SourcePriorityProperty(\n            primary_source="primarySource"\n        ),\n        state="state"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['failover_mode', 'source_priority', 'state']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.FailoverConfigProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.IngressGatewayBridgeProperty
class CfnBridge_IngressGatewayBridgePropertyDef(BaseStruct):
    max_bitrate: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The maximum expected bitrate (in bps) of the ingress bridge.\n')
    max_outputs: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The maximum number of outputs on the ingress bridge.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-ingressgatewaybridge.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    ingress_gateway_bridge_property = mediaconnect.CfnBridge.IngressGatewayBridgeProperty(\n        max_bitrate=123,\n        max_outputs=123\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['max_bitrate', 'max_outputs']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.IngressGatewayBridgeProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.SourcePriorityProperty
class CfnBridge_SourcePriorityPropertyDef(BaseStruct):
    primary_source: typing.Optional[str] = pydantic.Field(None, description='The name of the source you choose as the primary source for this flow.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-sourcepriority.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    source_priority_property = mediaconnect.CfnBridge.SourcePriorityProperty(\n        primary_source="primarySource"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['primary_source']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.SourcePriorityProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge.VpcInterfaceAttachmentProperty
class CfnBridge_VpcInterfaceAttachmentPropertyDef(BaseStruct):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridge-vpcinterfaceattachment.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    vpc_interface_attachment_property = mediaconnect.CfnBridge.VpcInterfaceAttachmentProperty(\n        vpc_interface_name="vpcInterfaceName"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['vpc_interface_name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge.VpcInterfaceAttachmentProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeOutput.BridgeNetworkOutputProperty
class CfnBridgeOutput_BridgeNetworkOutputPropertyDef(BaseStruct):
    ip_address: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output IP Address.\n')
    network_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description="The network output's gateway network name.\n")
    port: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output port.\n')
    protocol: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output protocol.\n')
    ttl: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output TTL.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridgeoutput-bridgenetworkoutput.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_network_output_property = mediaconnect.CfnBridgeOutput.BridgeNetworkOutputProperty(\n        ip_address="ipAddress",\n        network_name="networkName",\n        port=123,\n        protocol="protocol",\n        ttl=123\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['ip_address', 'network_name', 'port', 'protocol', 'ttl']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeOutput.BridgeNetworkOutputProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeSource.BridgeFlowSourceProperty
class CfnBridgeSource_BridgeFlowSourcePropertyDef(BaseStruct):
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the cloud flow used as a source of this bridge.\n')
    flow_vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeSource_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The name of the VPC interface attachment to use for this source.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridgesource-bridgeflowsource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_flow_source_property = mediaconnect.CfnBridgeSource.BridgeFlowSourceProperty(\n        flow_arn="flowArn",\n\n        # the properties below are optional\n        flow_vpc_interface_attachment=mediaconnect.CfnBridgeSource.VpcInterfaceAttachmentProperty(\n            vpc_interface_name="vpcInterfaceName"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['flow_arn', 'flow_vpc_interface_attachment']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeSource.BridgeFlowSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeSource.BridgeNetworkSourceProperty
class CfnBridgeSource_BridgeNetworkSourcePropertyDef(BaseStruct):
    multicast_ip: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network source multicast IP.\n')
    network_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description="The network source's gateway network name.\n")
    port: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network source port.\n')
    protocol: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network source protocol.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridgesource-bridgenetworksource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    bridge_network_source_property = mediaconnect.CfnBridgeSource.BridgeNetworkSourceProperty(\n        multicast_ip="multicastIp",\n        network_name="networkName",\n        port=123,\n        protocol="protocol"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['multicast_ip', 'network_name', 'port', 'protocol']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeSource.BridgeNetworkSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeSource.VpcInterfaceAttachmentProperty
class CfnBridgeSource_VpcInterfaceAttachmentPropertyDef(BaseStruct):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-bridgesource-vpcinterfaceattachment.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    vpc_interface_attachment_property = mediaconnect.CfnBridgeSource.VpcInterfaceAttachmentProperty(\n        vpc_interface_name="vpcInterfaceName"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['vpc_interface_name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeSource.VpcInterfaceAttachmentProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.EncryptionProperty
class CfnFlow_EncryptionPropertyDef(BaseStruct):
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the role that you created during setup (when you set up MediaConnect as a trusted entity).\n')
    algorithm: typing.Optional[str] = pydantic.Field(None, description='The type of algorithm that is used for static key encryption (such as aes128, aes192, or aes256). If you are using SPEKE or SRT-password encryption, this property must be left blank.\n')
    constant_initialization_vector: typing.Optional[str] = pydantic.Field(None, description='A 128-bit, 16-byte hex value represented by a 32-character string, to be used with the key for encrypting content. This parameter is not valid for static key encryption.\n')
    device_id: typing.Optional[str] = pydantic.Field(None, description='The value of one of the devices that you configured with your digital rights management (DRM) platform key provider. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    key_type: typing.Optional[str] = pydantic.Field(None, description='The type of key that is used for the encryption. If you don\'t specify a ``keyType`` value, the service uses the default setting ( ``static-key`` ). Valid key types are: ``static-key`` , ``speke`` , and ``srt-password`` . Default: - "static-key"\n')
    region: typing.Optional[str] = pydantic.Field(None, description='The AWS Region that the API Gateway proxy endpoint was created in. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    resource_id: typing.Optional[str] = pydantic.Field(None, description='An identifier for the content. The service sends this value to the key server to identify the current endpoint. The resource ID is also known as the content ID. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    secret_arn: typing.Optional[str] = pydantic.Field(None, description='The ARN of the secret that you created in AWS Secrets Manager to store the encryption key.\n')
    url: typing.Optional[str] = pydantic.Field(None, description='The URL from the API Gateway proxy that you set up to talk to your key server. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-encryption.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    encryption_property = mediaconnect.CfnFlow.EncryptionProperty(\n        role_arn="roleArn",\n\n        # the properties below are optional\n        algorithm="algorithm",\n        constant_initialization_vector="constantInitializationVector",\n        device_id="deviceId",\n        key_type="keyType",\n        region="region",\n        resource_id="resourceId",\n        secret_arn="secretArn",\n        url="url"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['role_arn', 'algorithm', 'constant_initialization_vector', 'device_id', 'key_type', 'region', 'resource_id', 'secret_arn', 'url']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.EncryptionProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.FailoverConfigProperty
class CfnFlow_FailoverConfigPropertyDef(BaseStruct):
    failover_mode: typing.Optional[str] = pydantic.Field(None, description='The type of failover you choose for this flow. MERGE combines the source streams into a single stream, allowing graceful recovery from any single-source loss. FAILOVER allows switching between different streams. The string for this property must be entered as MERGE or FAILOVER. No other string entry is valid.\n')
    recovery_window: typing.Union[int, float, None] = pydantic.Field(None, description='The size of the buffer (delay) that the service maintains. A larger buffer means a longer delay in transmitting the stream, but more room for error correction. A smaller buffer means a shorter delay, but less room for error correction. You can choose a value from 100-500 ms. If you keep this field blank, the service uses the default value of 200 ms. This setting only applies when Failover Mode is set to MERGE.\n')
    source_priority: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_SourcePriorityPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The priority you want to assign to a source. You can have a primary stream and a backup stream or two equally prioritized streams. This setting only applies when Failover Mode is set to FAILOVER.\n')
    state: typing.Optional[str] = pydantic.Field(None, description='The state of source failover on the flow. If the state is inactive, the flow can have only one source. If the state is active, the flow can have one or two sources.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-failoverconfig.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    failover_config_property = mediaconnect.CfnFlow.FailoverConfigProperty(\n        failover_mode="failoverMode",\n        recovery_window=123,\n        source_priority=mediaconnect.CfnFlow.SourcePriorityProperty(\n            primary_source="primarySource"\n        ),\n        state="state"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['failover_mode', 'recovery_window', 'source_priority', 'state']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.FailoverConfigProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.FmtpProperty
class CfnFlow_FmtpPropertyDef(BaseStruct):
    channel_order: typing.Optional[str] = pydantic.Field(None, description='The format of the audio channel.\n')
    colorimetry: typing.Optional[str] = pydantic.Field(None, description='The format used for the representation of color.\n')
    exact_framerate: typing.Optional[str] = pydantic.Field(None, description='The frame rate for the video stream, in frames/second. For example: 60000/1001.\n')
    par: typing.Optional[str] = pydantic.Field(None, description='The pixel aspect ratio (PAR) of the video.\n')
    range: typing.Optional[str] = pydantic.Field(None, description='The encoding range of the video.\n')
    scan_mode: typing.Optional[str] = pydantic.Field(None, description='The type of compression that was used to smooth the video’s appearance.\n')
    tcs: typing.Optional[str] = pydantic.Field(None, description='The transfer characteristic system (TCS) that is used in the video.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-fmtp.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    fmtp_property = mediaconnect.CfnFlow.FmtpProperty(\n        channel_order="channelOrder",\n        colorimetry="colorimetry",\n        exact_framerate="exactFramerate",\n        par="par",\n        range="range",\n        scan_mode="scanMode",\n        tcs="tcs"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['channel_order', 'colorimetry', 'exact_framerate', 'par', 'range', 'scan_mode', 'tcs']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.FmtpProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.GatewayBridgeSourceProperty
class CfnFlow_GatewayBridgeSourcePropertyDef(BaseStruct):
    bridge_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the bridge feeding this flow.\n')
    vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The name of the VPC interface attachment to use for this bridge source.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-gatewaybridgesource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    gateway_bridge_source_property = mediaconnect.CfnFlow.GatewayBridgeSourceProperty(\n        bridge_arn="bridgeArn",\n\n        # the properties below are optional\n        vpc_interface_attachment=mediaconnect.CfnFlow.VpcInterfaceAttachmentProperty(\n            vpc_interface_name="vpcInterfaceName"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['bridge_arn', 'vpc_interface_attachment']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.GatewayBridgeSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.InputConfigurationProperty
class CfnFlow_InputConfigurationPropertyDef(BaseStruct):
    input_port: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The port that the flow listens on for an incoming media stream.\n')
    interface: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_InterfacePropertyDef, dict[str, typing.Any]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The VPC interface where the media stream comes in from.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-inputconfiguration.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    input_configuration_property = mediaconnect.CfnFlow.InputConfigurationProperty(\n        input_port=123,\n        interface=mediaconnect.CfnFlow.InterfaceProperty(\n            name="name"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['input_port', 'interface']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.InputConfigurationProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.InterfaceProperty
class CfnFlow_InterfacePropertyDef(BaseStruct):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the VPC interface that you want to use for the media stream associated with the output.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-interface.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    interface_property = mediaconnect.CfnFlow.InterfaceProperty(\n        name="name"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.InterfaceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.MaintenanceProperty
class CfnFlow_MaintenancePropertyDef(BaseStruct):
    maintenance_day: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A day of a week when the maintenance will happen. Use Monday/Tuesday/Wednesday/Thursday/Friday/Saturday/Sunday.\n')
    maintenance_start_hour: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='UTC time when the maintenance will happen. Use 24-hour HH:MM format. Minutes must be 00. Example: 13:00. The default value is 02:00.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-maintenance.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    maintenance_property = mediaconnect.CfnFlow.MaintenanceProperty(\n        maintenance_day="maintenanceDay",\n        maintenance_start_hour="maintenanceStartHour"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['maintenance_day', 'maintenance_start_hour']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.MaintenanceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.MediaStreamAttributesProperty
class CfnFlow_MediaStreamAttributesPropertyDef(BaseStruct):
    fmtp: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_FmtpPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='A set of parameters that define the media stream.\n')
    lang: typing.Optional[str] = pydantic.Field(None, description='The audio language, in a format that is recognized by the receiver.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-mediastreamattributes.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    media_stream_attributes_property = mediaconnect.CfnFlow.MediaStreamAttributesProperty(\n        fmtp=mediaconnect.CfnFlow.FmtpProperty(\n            channel_order="channelOrder",\n            colorimetry="colorimetry",\n            exact_framerate="exactFramerate",\n            par="par",\n            range="range",\n            scan_mode="scanMode",\n            tcs="tcs"\n        ),\n        lang="lang"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['fmtp', 'lang']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.MediaStreamAttributesProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.MediaStreamProperty
class CfnFlow_MediaStreamPropertyDef(BaseStruct):
    media_stream_id: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='A unique identifier for the media stream.\n')
    media_stream_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A name that helps you distinguish one media stream from another.\n')
    media_stream_type: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The type of media stream.\n')
    attributes: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MediaStreamAttributesPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Attributes that are related to the media stream.\n')
    clock_rate: typing.Union[int, float, None] = pydantic.Field(None, description='The sample rate for the stream. This value in measured in kHz.\n')
    description: typing.Optional[str] = pydantic.Field(None, description='A description that can help you quickly identify what your media stream is used for.\n')
    fmt: typing.Union[int, float, None] = pydantic.Field(None, description='The format type number (sometimes referred to as RTP payload type) of the media stream. MediaConnect assigns this value to the media stream. For ST 2110 JPEG XS outputs, you need to provide this value to the receiver.\n')
    video_format: typing.Optional[str] = pydantic.Field(None, description='The resolution of the video.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-mediastream.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    media_stream_property = mediaconnect.CfnFlow.MediaStreamProperty(\n        media_stream_id=123,\n        media_stream_name="mediaStreamName",\n        media_stream_type="mediaStreamType",\n\n        # the properties below are optional\n        attributes=mediaconnect.CfnFlow.MediaStreamAttributesProperty(\n            fmtp=mediaconnect.CfnFlow.FmtpProperty(\n                channel_order="channelOrder",\n                colorimetry="colorimetry",\n                exact_framerate="exactFramerate",\n                par="par",\n                range="range",\n                scan_mode="scanMode",\n                tcs="tcs"\n            ),\n            lang="lang"\n        ),\n        clock_rate=123,\n        description="description",\n        fmt=123,\n        video_format="videoFormat"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['media_stream_id', 'media_stream_name', 'media_stream_type', 'attributes', 'clock_rate', 'description', 'fmt', 'video_format']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.MediaStreamProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.MediaStreamSourceConfigurationProperty
class CfnFlow_MediaStreamSourceConfigurationPropertyDef(BaseStruct):
    encoding_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The format that was used to encode the data. For ancillary data streams, set the encoding name to ``smpte291`` . For audio streams, set the encoding name to ``pcm`` . For video, 2110 streams, set the encoding name to ``raw`` . For video, JPEG XS streams, set the encoding name to ``jxsv`` .\n')
    media_stream_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A name that helps you distinguish one media stream from another.\n')
    input_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_InputConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The media streams that you want to associate with the source.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-mediastreamsourceconfiguration.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    media_stream_source_configuration_property = mediaconnect.CfnFlow.MediaStreamSourceConfigurationProperty(\n        encoding_name="encodingName",\n        media_stream_name="mediaStreamName",\n\n        # the properties below are optional\n        input_configurations=[mediaconnect.CfnFlow.InputConfigurationProperty(\n            input_port=123,\n            interface=mediaconnect.CfnFlow.InterfaceProperty(\n                name="name"\n            )\n        )]\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['encoding_name', 'media_stream_name', 'input_configurations']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.MediaStreamSourceConfigurationProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.SourcePriorityProperty
class CfnFlow_SourcePriorityPropertyDef(BaseStruct):
    primary_source: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the source you choose as the primary source for this flow.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-sourcepriority.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    source_priority_property = mediaconnect.CfnFlow.SourcePriorityProperty(\n        primary_source="primarySource"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['primary_source']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.SourcePriorityProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.SourceProperty
class CfnFlow_SourcePropertyDef(BaseStruct):
    decryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The type of encryption that is used on the content ingested from the source.\n')
    description: typing.Optional[str] = pydantic.Field(None, description='A description of the source. This description is not visible outside of the current AWS account.\n')
    entitlement_arn: typing.Optional[str] = pydantic.Field(None, description='The ARN of the entitlement that allows you to subscribe to content that comes from another AWS account. The entitlement is set by the content originator and the ARN is generated as part of the originator’s flow.\n')
    gateway_bridge_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_GatewayBridgeSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The source configuration for cloud flows receiving a stream from a bridge.\n')
    ingest_ip: typing.Optional[str] = pydantic.Field(None, description='The IP address that the flow listens on for incoming content.\n')
    ingest_port: typing.Union[int, float, None] = pydantic.Field(None, description='The port that the flow listens on for incoming content. If the protocol of the source is Zixi, the port must be set to 2088.\n')
    max_bitrate: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum bitrate for RIST, RTP, and RTP-FEC streams.\n')
    max_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum latency in milliseconds for a RIST or Zixi-based source.\n')
    max_sync_buffer: typing.Union[int, float, None] = pydantic.Field(None, description='The size of the buffer (in milliseconds) to use to sync incoming source data.\n')
    media_stream_source_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MediaStreamSourceConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The media stream that is associated with the source, and the parameters for that association.\n')
    min_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The minimum latency in milliseconds for SRT-based streams. In streams that use the SRT protocol, this value that you set on your MediaConnect source or output represents the minimal potential latency of that connection. The latency of the stream is set to the highest number between the sender’s minimum latency and the receiver’s minimum latency.\n')
    name: typing.Optional[str] = pydantic.Field(None, description='The name of the source.\n')
    protocol: typing.Optional[str] = pydantic.Field(None, description='The protocol that is used by the source. AWS CloudFormation does not currently support CDI or ST 2110 JPEG XS source protocols.\n')
    sender_control_port: typing.Union[int, float, None] = pydantic.Field(None, description='The port that the flow uses to send outbound requests to initiate connection with the sender.\n')
    sender_ip_address: typing.Optional[str] = pydantic.Field(None, description='The IP address that the flow communicates with to initiate connection with the sender.\n')
    source_arn: typing.Optional[str] = pydantic.Field(None, description='The ARN of the source.\n')
    source_ingest_port: typing.Optional[str] = pydantic.Field(None, description='The port that the flow listens on for incoming content. If the protocol of the source is Zixi, the port must be set to 2088.\n')
    source_listener_address: typing.Optional[str] = pydantic.Field(None, description='Source IP or domain name for SRT-caller protocol.\n')
    source_listener_port: typing.Union[int, float, None] = pydantic.Field(None, description='Source port for SRT-caller protocol.\n')
    stream_id: typing.Optional[str] = pydantic.Field(None, description='The stream ID that you want to use for the transport. This parameter applies only to Zixi-based streams.\n')
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that the source content comes from.\n')
    whitelist_cidr: typing.Optional[str] = pydantic.Field(None, description='The range of IP addresses that are allowed to contribute content to your source. Format the IP addresses as a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-source.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    source_property = mediaconnect.CfnFlow.SourceProperty(\n        decryption=mediaconnect.CfnFlow.EncryptionProperty(\n            role_arn="roleArn",\n\n            # the properties below are optional\n            algorithm="algorithm",\n            constant_initialization_vector="constantInitializationVector",\n            device_id="deviceId",\n            key_type="keyType",\n            region="region",\n            resource_id="resourceId",\n            secret_arn="secretArn",\n            url="url"\n        ),\n        description="description",\n        entitlement_arn="entitlementArn",\n        gateway_bridge_source=mediaconnect.CfnFlow.GatewayBridgeSourceProperty(\n            bridge_arn="bridgeArn",\n\n            # the properties below are optional\n            vpc_interface_attachment=mediaconnect.CfnFlow.VpcInterfaceAttachmentProperty(\n                vpc_interface_name="vpcInterfaceName"\n            )\n        ),\n        ingest_ip="ingestIp",\n        ingest_port=123,\n        max_bitrate=123,\n        max_latency=123,\n        max_sync_buffer=123,\n        media_stream_source_configurations=[mediaconnect.CfnFlow.MediaStreamSourceConfigurationProperty(\n            encoding_name="encodingName",\n            media_stream_name="mediaStreamName",\n\n            # the properties below are optional\n            input_configurations=[mediaconnect.CfnFlow.InputConfigurationProperty(\n                input_port=123,\n                interface=mediaconnect.CfnFlow.InterfaceProperty(\n                    name="name"\n                )\n            )]\n        )],\n        min_latency=123,\n        name="name",\n        protocol="protocol",\n        sender_control_port=123,\n        sender_ip_address="senderIpAddress",\n        source_arn="sourceArn",\n        source_ingest_port="sourceIngestPort",\n        source_listener_address="sourceListenerAddress",\n        source_listener_port=123,\n        stream_id="streamId",\n        vpc_interface_name="vpcInterfaceName",\n        whitelist_cidr="whitelistCidr"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['decryption', 'description', 'entitlement_arn', 'gateway_bridge_source', 'ingest_ip', 'ingest_port', 'max_bitrate', 'max_latency', 'max_sync_buffer', 'media_stream_source_configurations', 'min_latency', 'name', 'protocol', 'sender_control_port', 'sender_ip_address', 'source_arn', 'source_ingest_port', 'source_listener_address', 'source_listener_port', 'stream_id', 'vpc_interface_name', 'whitelist_cidr']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.SourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.VpcInterfaceAttachmentProperty
class CfnFlow_VpcInterfaceAttachmentPropertyDef(BaseStruct):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-vpcinterfaceattachment.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    vpc_interface_attachment_property = mediaconnect.CfnFlow.VpcInterfaceAttachmentProperty(\n        vpc_interface_name="vpcInterfaceName"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['vpc_interface_name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.VpcInterfaceAttachmentProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow.VpcInterfaceProperty
class CfnFlow_VpcInterfacePropertyDef(BaseStruct):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name for the VPC interface. This name must be unique within the flow.\n')
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the IAM role that you created when you set up MediaConnect as a trusted service.\n')
    security_group_ids: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A virtual firewall to control inbound and outbound traffic.\n')
    subnet_id: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The subnet IDs that you specified for your VPC interface. A subnet ID is a range of IP addresses in your VPC. When you create your VPC, you specify a range of IPv4 addresses for the VPC in the form of a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16. This is the primary CIDR block for your VPC. When you create a subnet for your VPC, you specify the CIDR block for the subnet, which is a subset of the VPC CIDR block. The subnets that you use across all VPC interfaces on the flow must be in the same Availability Zone as the flow.\n')
    network_interface_ids: typing.Optional[typing.Sequence[str]] = pydantic.Field(None, description='The IDs of the network interfaces that MediaConnect created in your account.\n')
    network_interface_type: typing.Optional[str] = pydantic.Field(None, description='The type of network interface.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flow-vpcinterface.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    vpc_interface_property = mediaconnect.CfnFlow.VpcInterfaceProperty(\n        name="name",\n        role_arn="roleArn",\n        security_group_ids=["securityGroupIds"],\n        subnet_id="subnetId",\n\n        # the properties below are optional\n        network_interface_ids=["networkInterfaceIds"],\n        network_interface_type="networkInterfaceType"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['name', 'role_arn', 'security_group_ids', 'subnet_id', 'network_interface_ids', 'network_interface_type']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow.VpcInterfaceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowEntitlement.EncryptionProperty
class CfnFlowEntitlement_EncryptionPropertyDef(BaseStruct):
    algorithm: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The type of algorithm that is used for static key encryption (such as aes128, aes192, or aes256). If you are using SPEKE or SRT-password encryption, this property must be left blank.\n')
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the role that you created during setup (when you set up MediaConnect as a trusted entity).\n')
    constant_initialization_vector: typing.Optional[str] = pydantic.Field(None, description='A 128-bit, 16-byte hex value represented by a 32-character string, to be used with the key for encrypting content. This parameter is not valid for static key encryption.\n')
    device_id: typing.Optional[str] = pydantic.Field(None, description='The value of one of the devices that you configured with your digital rights management (DRM) platform key provider. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    key_type: typing.Optional[str] = pydantic.Field(None, description='The type of key that is used for the encryption. If you don\'t specify a ``keyType`` value, the service uses the default setting ( ``static-key`` ). Valid key types are: ``static-key`` , ``speke`` , and ``srt-password`` . Default: - "static-key"\n')
    region: typing.Optional[str] = pydantic.Field(None, description='The AWS Region that the API Gateway proxy endpoint was created in. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    resource_id: typing.Optional[str] = pydantic.Field(None, description='An identifier for the content. The service sends this value to the key server to identify the current endpoint. The resource ID is also known as the content ID. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    secret_arn: typing.Optional[str] = pydantic.Field(None, description='The ARN of the secret that you created in AWS Secrets Manager to store the encryption key.\n')
    url: typing.Optional[str] = pydantic.Field(None, description='The URL from the API Gateway proxy that you set up to talk to your key server. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowentitlement-encryption.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    encryption_property = mediaconnect.CfnFlowEntitlement.EncryptionProperty(\n        algorithm="algorithm",\n        role_arn="roleArn",\n\n        # the properties below are optional\n        constant_initialization_vector="constantInitializationVector",\n        device_id="deviceId",\n        key_type="keyType",\n        region="region",\n        resource_id="resourceId",\n        secret_arn="secretArn",\n        url="url"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['algorithm', 'role_arn', 'constant_initialization_vector', 'device_id', 'key_type', 'region', 'resource_id', 'secret_arn', 'url']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowEntitlement.EncryptionProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput.DestinationConfigurationProperty
class CfnFlowOutput_DestinationConfigurationPropertyDef(BaseStruct):
    destination_ip: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The IP address where contents of the media stream will be sent.\n')
    destination_port: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='The port to use when the content of the media stream is distributed to the output.\n')
    interface: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_InterfacePropertyDef, dict[str, typing.Any]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The VPC interface that is used for the media stream associated with the output.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowoutput-destinationconfiguration.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    destination_configuration_property = mediaconnect.CfnFlowOutput.DestinationConfigurationProperty(\n        destination_ip="destinationIp",\n        destination_port=123,\n        interface=mediaconnect.CfnFlowOutput.InterfaceProperty(\n            name="name"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['destination_ip', 'destination_port', 'interface']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput.DestinationConfigurationProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput.EncodingParametersProperty
class CfnFlowOutput_EncodingParametersPropertyDef(BaseStruct):
    compression_factor: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='A value that is used to calculate compression for an output. The bitrate of the output is calculated as follows: Output bitrate = (1 / compressionFactor) * (source bitrate) This property only applies to outputs that use the ST 2110 JPEG XS protocol, with a flow source that uses the CDI protocol. Valid values are in the range of 3.0 to 10.0, inclusive.\n')
    encoder_profile: typing.Optional[str] = pydantic.Field(None, description='A setting on the encoder that drives compression settings. This property only applies to video media streams associated with outputs that use the ST 2110 JPEG XS protocol, with a flow source that uses the CDI protocol.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowoutput-encodingparameters.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    encoding_parameters_property = mediaconnect.CfnFlowOutput.EncodingParametersProperty(\n        compression_factor=123,\n\n        # the properties below are optional\n        encoder_profile="encoderProfile"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['compression_factor', 'encoder_profile']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput.EncodingParametersProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput.EncryptionProperty
class CfnFlowOutput_EncryptionPropertyDef(BaseStruct):
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the role that you created during setup (when you set up MediaConnect as a trusted entity).\n')
    secret_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the secret that you created in AWS Secrets Manager to store the encryption key.\n')
    algorithm: typing.Optional[str] = pydantic.Field(None, description='The type of algorithm that is used for static key encryption (such as aes128, aes192, or aes256). If you are using SPEKE or SRT-password encryption, this property must be left blank.\n')
    key_type: typing.Optional[str] = pydantic.Field(None, description='The type of key that is used for the encryption. If you don\'t specify a ``keyType`` value, the service uses the default setting ( ``static-key`` ). Valid key types are: ``static-key`` , ``speke`` , and ``srt-password`` . Default: - "static-key"\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowoutput-encryption.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    encryption_property = mediaconnect.CfnFlowOutput.EncryptionProperty(\n        role_arn="roleArn",\n        secret_arn="secretArn",\n\n        # the properties below are optional\n        algorithm="algorithm",\n        key_type="keyType"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['role_arn', 'secret_arn', 'algorithm', 'key_type']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput.EncryptionProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput.InterfaceProperty
class CfnFlowOutput_InterfacePropertyDef(BaseStruct):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the VPC interface that you want to use for the media stream associated with the output.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowoutput-interface.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    interface_property = mediaconnect.CfnFlowOutput.InterfaceProperty(\n        name="name"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput.InterfaceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput.MediaStreamOutputConfigurationProperty
class CfnFlowOutput_MediaStreamOutputConfigurationPropertyDef(BaseStruct):
    encoding_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The format that will be used to encode the data. For ancillary data streams, set the encoding name to ``smpte291`` . For audio streams, set the encoding name to ``pcm`` . For video streams on sources or outputs that use the CDI protocol, set the encoding name to ``raw`` . For video streams on sources or outputs that use the ST 2110 JPEG XS protocol, set the encoding name to ``jxsv`` .\n')
    media_stream_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A name that helps you distinguish one media stream from another.\n')
    destination_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_DestinationConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The media streams that you want to associate with the output.\n')
    encoding_parameters: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_EncodingParametersPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='A collection of parameters that determine how MediaConnect will convert the content. These fields only apply to outputs on flows that have a CDI source.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowoutput-mediastreamoutputconfiguration.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    media_stream_output_configuration_property = mediaconnect.CfnFlowOutput.MediaStreamOutputConfigurationProperty(\n        encoding_name="encodingName",\n        media_stream_name="mediaStreamName",\n\n        # the properties below are optional\n        destination_configurations=[mediaconnect.CfnFlowOutput.DestinationConfigurationProperty(\n            destination_ip="destinationIp",\n            destination_port=123,\n            interface=mediaconnect.CfnFlowOutput.InterfaceProperty(\n                name="name"\n            )\n        )],\n        encoding_parameters=mediaconnect.CfnFlowOutput.EncodingParametersProperty(\n            compression_factor=123,\n\n            # the properties below are optional\n            encoder_profile="encoderProfile"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['encoding_name', 'media_stream_name', 'destination_configurations', 'encoding_parameters']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput.MediaStreamOutputConfigurationProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput.VpcInterfaceAttachmentProperty
class CfnFlowOutput_VpcInterfaceAttachmentPropertyDef(BaseStruct):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowoutput-vpcinterfaceattachment.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    vpc_interface_attachment_property = mediaconnect.CfnFlowOutput.VpcInterfaceAttachmentProperty(\n        vpc_interface_name="vpcInterfaceName"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['vpc_interface_name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput.VpcInterfaceAttachmentProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowSource.EncryptionProperty
class CfnFlowSource_EncryptionPropertyDef(BaseStruct):
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the role that you created during setup (when you set up MediaConnect as a trusted entity).\n')
    algorithm: typing.Optional[str] = pydantic.Field(None, description='The type of algorithm that is used for static key encryption (such as aes128, aes192, or aes256). If you are using SPEKE or SRT-password encryption, this property must be left blank.\n')
    constant_initialization_vector: typing.Optional[str] = pydantic.Field(None, description='A 128-bit, 16-byte hex value represented by a 32-character string, to be used with the key for encrypting content. This parameter is not valid for static key encryption.\n')
    device_id: typing.Optional[str] = pydantic.Field(None, description='The value of one of the devices that you configured with your digital rights management (DRM) platform key provider. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    key_type: typing.Optional[str] = pydantic.Field(None, description='The type of key that is used for the encryption. If you don\'t specify a ``keyType`` value, the service uses the default setting ( ``static-key`` ). Valid key types are: ``static-key`` , ``speke`` , and ``srt-password`` . Default: - "static-key"\n')
    region: typing.Optional[str] = pydantic.Field(None, description='The AWS Region that the API Gateway proxy endpoint was created in. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    resource_id: typing.Optional[str] = pydantic.Field(None, description='An identifier for the content. The service sends this value to the key server to identify the current endpoint. The resource ID is also known as the content ID. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n')
    secret_arn: typing.Optional[str] = pydantic.Field(None, description='The ARN of the secret that you created in AWS Secrets Manager to store the encryption key.\n')
    url: typing.Optional[str] = pydantic.Field(None, description='The URL from the API Gateway proxy that you set up to talk to your key server. This parameter is required for SPEKE encryption and is not valid for static key encryption.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowsource-encryption.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    encryption_property = mediaconnect.CfnFlowSource.EncryptionProperty(\n        role_arn="roleArn",\n\n        # the properties below are optional\n        algorithm="algorithm",\n        constant_initialization_vector="constantInitializationVector",\n        device_id="deviceId",\n        key_type="keyType",\n        region="region",\n        resource_id="resourceId",\n        secret_arn="secretArn",\n        url="url"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['role_arn', 'algorithm', 'constant_initialization_vector', 'device_id', 'key_type', 'region', 'resource_id', 'secret_arn', 'url']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowSource.EncryptionProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowSource.GatewayBridgeSourceProperty
class CfnFlowSource_GatewayBridgeSourcePropertyDef(BaseStruct):
    bridge_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the bridge feeding this flow.\n')
    vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowSource_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The name of the VPC interface attachment to use for this bridge source.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowsource-gatewaybridgesource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    gateway_bridge_source_property = mediaconnect.CfnFlowSource.GatewayBridgeSourceProperty(\n        bridge_arn="bridgeArn",\n\n        # the properties below are optional\n        vpc_interface_attachment=mediaconnect.CfnFlowSource.VpcInterfaceAttachmentProperty(\n            vpc_interface_name="vpcInterfaceName"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['bridge_arn', 'vpc_interface_attachment']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowSource.GatewayBridgeSourceProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowSource.VpcInterfaceAttachmentProperty
class CfnFlowSource_VpcInterfaceAttachmentPropertyDef(BaseStruct):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-flowsource-vpcinterfaceattachment.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    vpc_interface_attachment_property = mediaconnect.CfnFlowSource.VpcInterfaceAttachmentProperty(\n        vpc_interface_name="vpcInterfaceName"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['vpc_interface_name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowSource.VpcInterfaceAttachmentProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnGateway.GatewayNetworkProperty
class CfnGateway_GatewayNetworkPropertyDef(BaseStruct):
    cidr_block: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A unique IP address range to use for this network. These IP addresses should be in the form of a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the network. This name is used to reference the network and must be unique among networks in this gateway.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-mediaconnect-gateway-gatewaynetwork.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    gateway_network_property = mediaconnect.CfnGateway.GatewayNetworkProperty(\n        cidr_block="cidrBlock",\n        name="name"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['cidr_block', 'name']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnGateway.GatewayNetworkProperty'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridge
class CfnBridgeDef(BaseCfnResource):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the bridge. This name can not be modified after the bridge is created.\n')
    placement_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The bridge placement Amazon Resource Number (ARN).\n')
    sources: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeSourcePropertyDef, dict[str, typing.Any]]]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The sources that you want to add to this bridge.\n')
    egress_gateway_bridge: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_EgressGatewayBridgePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Create a bridge with the egress bridge type. An egress bridge is a cloud-to-ground bridge. The content comes from an existing MediaConnect flow and is delivered to your premises.\n')
    ingress_gateway_bridge: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_IngressGatewayBridgePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Create a bridge with the ingress bridge type. An ingress bridge is a ground-to-cloud bridge. The content originates at your premises and is delivered to the cloud.\n')
    outputs: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeOutputPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The outputs that you want to add to this bridge.\n')
    source_failover_config: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_FailoverConfigPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The settings for source failover.')
    _init_params: typing.ClassVar[list[str]] = ['name', 'placement_arn', 'sources', 'egress_gateway_bridge', 'ingress_gateway_bridge', 'outputs', 'source_failover_config']
    _method_names: typing.ClassVar[list[str]] = ['BridgeFlowSourceProperty', 'BridgeNetworkOutputProperty', 'BridgeNetworkSourceProperty', 'BridgeOutputProperty', 'BridgeSourceProperty', 'EgressGatewayBridgeProperty', 'FailoverConfigProperty', 'IngressGatewayBridgeProperty', 'SourcePriorityProperty', 'VpcInterfaceAttachmentProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridge'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnBridgeDefConfig] = pydantic.Field(None)


class CfnBridgeDefConfig(pydantic.BaseModel):
    BridgeFlowSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefBridgeflowsourcepropertyParams]] = pydantic.Field(None, description='')
    BridgeNetworkOutputProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefBridgenetworkoutputpropertyParams]] = pydantic.Field(None, description='')
    BridgeNetworkSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefBridgenetworksourcepropertyParams]] = pydantic.Field(None, description='')
    BridgeOutputProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefBridgeoutputpropertyParams]] = pydantic.Field(None, description='')
    BridgeSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefBridgesourcepropertyParams]] = pydantic.Field(None, description='')
    EgressGatewayBridgeProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefEgressgatewaybridgepropertyParams]] = pydantic.Field(None, description='')
    FailoverConfigProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefFailoverconfigpropertyParams]] = pydantic.Field(None, description='')
    IngressGatewayBridgeProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefIngressgatewaybridgepropertyParams]] = pydantic.Field(None, description='')
    SourcePriorityProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefSourceprioritypropertyParams]] = pydantic.Field(None, description='')
    VpcInterfaceAttachmentProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefVpcinterfaceattachmentpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnBridgeDefBridgeflowsourcepropertyParams(pydantic.BaseModel):
    flow_arn: str = pydantic.Field(..., description='')
    name: str = pydantic.Field(..., description='')
    flow_vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnBridgeDefBridgenetworkoutputpropertyParams(pydantic.BaseModel):
    ip_address: str = pydantic.Field(..., description='')
    name: str = pydantic.Field(..., description='')
    network_name: str = pydantic.Field(..., description='')
    port: typing.Union[int, float] = pydantic.Field(..., description='')
    protocol: str = pydantic.Field(..., description='')
    ttl: typing.Union[int, float] = pydantic.Field(..., description='')
    ...

class CfnBridgeDefBridgenetworksourcepropertyParams(pydantic.BaseModel):
    multicast_ip: str = pydantic.Field(..., description='')
    name: str = pydantic.Field(..., description='')
    network_name: str = pydantic.Field(..., description='')
    port: typing.Union[int, float] = pydantic.Field(..., description='')
    protocol: str = pydantic.Field(..., description='')
    ...

class CfnBridgeDefBridgeoutputpropertyParams(pydantic.BaseModel):
    network_output: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeNetworkOutputPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnBridgeDefBridgesourcepropertyParams(pydantic.BaseModel):
    flow_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeFlowSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    network_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeNetworkSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnBridgeDefEgressgatewaybridgepropertyParams(pydantic.BaseModel):
    max_bitrate: typing.Union[int, float] = pydantic.Field(..., description='')
    ...

class CfnBridgeDefFailoverconfigpropertyParams(pydantic.BaseModel):
    failover_mode: str = pydantic.Field(..., description='')
    source_priority: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_SourcePriorityPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    state: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnBridgeDefIngressgatewaybridgepropertyParams(pydantic.BaseModel):
    max_bitrate: typing.Union[int, float] = pydantic.Field(..., description='')
    max_outputs: typing.Union[int, float] = pydantic.Field(..., description='')
    ...

class CfnBridgeDefSourceprioritypropertyParams(pydantic.BaseModel):
    primary_source: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnBridgeDefVpcinterfaceattachmentpropertyParams(pydantic.BaseModel):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnBridgeDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnBridgeDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnBridgeDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnBridgeDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnBridgeDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnBridgeDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnBridgeDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnBridgeDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnBridgeDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnBridgeDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnBridgeDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnBridgeDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnBridgeDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnBridgeDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeOutput
class CfnBridgeOutputDef(BaseCfnResource):
    bridge_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the bridge that you want to describe.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output name. This name is used to reference the output and must be unique among outputs in this bridge.\n')
    network_output: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeOutput_BridgeNetworkOutputPropertyDef, dict[str, typing.Any]] = pydantic.Field(REQUIRED_INIT_PARAM, description='Add a network output to an existing bridge.')
    _init_params: typing.ClassVar[list[str]] = ['bridge_arn', 'name', 'network_output']
    _method_names: typing.ClassVar[list[str]] = ['BridgeNetworkOutputProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeOutput'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnBridgeOutputDefConfig] = pydantic.Field(None)


class CfnBridgeOutputDefConfig(pydantic.BaseModel):
    BridgeNetworkOutputProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefBridgenetworkoutputpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeOutputDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnBridgeOutputDefBridgenetworkoutputpropertyParams(pydantic.BaseModel):
    ip_address: str = pydantic.Field(..., description='')
    network_name: str = pydantic.Field(..., description='')
    port: typing.Union[int, float] = pydantic.Field(..., description='')
    protocol: str = pydantic.Field(..., description='')
    ttl: typing.Union[int, float] = pydantic.Field(..., description='')
    ...

class CfnBridgeOutputDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnBridgeOutputDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnBridgeOutputDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnBridgeOutputDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnBridgeOutputDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnBridgeOutputDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnBridgeOutputDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnBridgeOutputDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnBridgeOutputDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnBridgeOutputDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnBridgeOutputDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnBridgeOutputDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnBridgeOutputDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnBridgeOutputDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeSource
class CfnBridgeSourceDef(BaseCfnResource):
    bridge_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the bridge that you want to describe.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the flow source. This name is used to reference the source and must be unique among sources in this bridge.\n')
    flow_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeSource_BridgeFlowSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Add a flow source to an existing bridge.\n')
    network_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeSource_BridgeNetworkSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Add a network source to an existing bridge.')
    _init_params: typing.ClassVar[list[str]] = ['bridge_arn', 'name', 'flow_source', 'network_source']
    _method_names: typing.ClassVar[list[str]] = ['BridgeFlowSourceProperty', 'BridgeNetworkSourceProperty', 'VpcInterfaceAttachmentProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeSource'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnBridgeSourceDefConfig] = pydantic.Field(None)


class CfnBridgeSourceDefConfig(pydantic.BaseModel):
    BridgeFlowSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefBridgeflowsourcepropertyParams]] = pydantic.Field(None, description='')
    BridgeNetworkSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefBridgenetworksourcepropertyParams]] = pydantic.Field(None, description='')
    VpcInterfaceAttachmentProperty: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefVpcinterfaceattachmentpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnBridgeSourceDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnBridgeSourceDefBridgeflowsourcepropertyParams(pydantic.BaseModel):
    flow_arn: str = pydantic.Field(..., description='')
    flow_vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeSource_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnBridgeSourceDefBridgenetworksourcepropertyParams(pydantic.BaseModel):
    multicast_ip: str = pydantic.Field(..., description='')
    network_name: str = pydantic.Field(..., description='')
    port: typing.Union[int, float] = pydantic.Field(..., description='')
    protocol: str = pydantic.Field(..., description='')
    ...

class CfnBridgeSourceDefVpcinterfaceattachmentpropertyParams(pydantic.BaseModel):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnBridgeSourceDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnBridgeSourceDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnBridgeSourceDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnBridgeSourceDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnBridgeSourceDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnBridgeSourceDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnBridgeSourceDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnBridgeSourceDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnBridgeSourceDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnBridgeSourceDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnBridgeSourceDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnBridgeSourceDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnBridgeSourceDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnBridgeSourceDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlow
class CfnFlowDef(BaseCfnResource):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the flow.\n')
    source: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_SourcePropertyDef, dict[str, typing.Any]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The settings for the source that you want to use for the new flow.\n')
    availability_zone: typing.Optional[str] = pydantic.Field(None, description='The Availability Zone that you want to create the flow in. These options are limited to the Availability Zones within the current AWS Region.\n')
    maintenance: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MaintenancePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The maintenance settings you want to use for the flow.\n')
    media_streams: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MediaStreamPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The media streams associated with the flow. You can associate any of these media streams with sources and outputs on the flow.\n')
    source_failover_config: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_FailoverConfigPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The settings for source failover.\n')
    vpc_interfaces: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_VpcInterfacePropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The VPC interfaces that you added to this flow.')
    _init_params: typing.ClassVar[list[str]] = ['name', 'source', 'availability_zone', 'maintenance', 'media_streams', 'source_failover_config', 'vpc_interfaces']
    _method_names: typing.ClassVar[list[str]] = ['EncryptionProperty', 'FailoverConfigProperty', 'FmtpProperty', 'GatewayBridgeSourceProperty', 'InputConfigurationProperty', 'InterfaceProperty', 'MaintenanceProperty', 'MediaStreamAttributesProperty', 'MediaStreamProperty', 'MediaStreamSourceConfigurationProperty', 'SourcePriorityProperty', 'SourceProperty', 'VpcInterfaceAttachmentProperty', 'VpcInterfaceProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlow'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnFlowDefConfig] = pydantic.Field(None)


class CfnFlowDefConfig(pydantic.BaseModel):
    EncryptionProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefEncryptionpropertyParams]] = pydantic.Field(None, description='')
    FailoverConfigProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefFailoverconfigpropertyParams]] = pydantic.Field(None, description='')
    FmtpProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefFmtppropertyParams]] = pydantic.Field(None, description='')
    GatewayBridgeSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefGatewaybridgesourcepropertyParams]] = pydantic.Field(None, description='')
    InputConfigurationProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefInputconfigurationpropertyParams]] = pydantic.Field(None, description='')
    InterfaceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefInterfacepropertyParams]] = pydantic.Field(None, description='')
    MaintenanceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefMaintenancepropertyParams]] = pydantic.Field(None, description='')
    MediaStreamAttributesProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefMediastreamattributespropertyParams]] = pydantic.Field(None, description='')
    MediaStreamProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefMediastreampropertyParams]] = pydantic.Field(None, description='')
    MediaStreamSourceConfigurationProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefMediastreamsourceconfigurationpropertyParams]] = pydantic.Field(None, description='')
    SourcePriorityProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefSourceprioritypropertyParams]] = pydantic.Field(None, description='')
    SourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefSourcepropertyParams]] = pydantic.Field(None, description='')
    VpcInterfaceAttachmentProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefVpcinterfaceattachmentpropertyParams]] = pydantic.Field(None, description='')
    VpcInterfaceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefVpcinterfacepropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnFlowDefEncryptionpropertyParams(pydantic.BaseModel):
    role_arn: str = pydantic.Field(..., description='')
    algorithm: typing.Optional[str] = pydantic.Field(None, description='')
    constant_initialization_vector: typing.Optional[str] = pydantic.Field(None, description='')
    device_id: typing.Optional[str] = pydantic.Field(None, description='')
    key_type: typing.Optional[str] = pydantic.Field(None, description='')
    region: typing.Optional[str] = pydantic.Field(None, description='')
    resource_id: typing.Optional[str] = pydantic.Field(None, description='')
    secret_arn: typing.Optional[str] = pydantic.Field(None, description='')
    url: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefFailoverconfigpropertyParams(pydantic.BaseModel):
    failover_mode: typing.Optional[str] = pydantic.Field(None, description='')
    recovery_window: typing.Union[int, float, None] = pydantic.Field(None, description='')
    source_priority: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_SourcePriorityPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    state: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefFmtppropertyParams(pydantic.BaseModel):
    channel_order: typing.Optional[str] = pydantic.Field(None, description='')
    colorimetry: typing.Optional[str] = pydantic.Field(None, description='')
    exact_framerate: typing.Optional[str] = pydantic.Field(None, description='')
    par: typing.Optional[str] = pydantic.Field(None, description='')
    range: typing.Optional[str] = pydantic.Field(None, description='')
    scan_mode: typing.Optional[str] = pydantic.Field(None, description='')
    tcs: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefGatewaybridgesourcepropertyParams(pydantic.BaseModel):
    bridge_arn: str = pydantic.Field(..., description='')
    vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnFlowDefInputconfigurationpropertyParams(pydantic.BaseModel):
    input_port: typing.Union[int, float] = pydantic.Field(..., description='')
    interface: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_InterfacePropertyDef, dict[str, typing.Any]] = pydantic.Field(..., description='')
    ...

class CfnFlowDefInterfacepropertyParams(pydantic.BaseModel):
    name: str = pydantic.Field(..., description='')
    ...

class CfnFlowDefMaintenancepropertyParams(pydantic.BaseModel):
    maintenance_day: str = pydantic.Field(..., description='')
    maintenance_start_hour: str = pydantic.Field(..., description='')
    ...

class CfnFlowDefMediastreamattributespropertyParams(pydantic.BaseModel):
    fmtp: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_FmtpPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    lang: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefMediastreampropertyParams(pydantic.BaseModel):
    media_stream_id: typing.Union[int, float] = pydantic.Field(..., description='')
    media_stream_name: str = pydantic.Field(..., description='')
    media_stream_type: str = pydantic.Field(..., description='')
    attributes: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MediaStreamAttributesPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    clock_rate: typing.Union[int, float, None] = pydantic.Field(None, description='')
    description: typing.Optional[str] = pydantic.Field(None, description='')
    fmt: typing.Union[int, float, None] = pydantic.Field(None, description='')
    video_format: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefMediastreamsourceconfigurationpropertyParams(pydantic.BaseModel):
    encoding_name: str = pydantic.Field(..., description='')
    media_stream_name: str = pydantic.Field(..., description='')
    input_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_InputConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='')
    ...

class CfnFlowDefSourceprioritypropertyParams(pydantic.BaseModel):
    primary_source: str = pydantic.Field(..., description='')
    ...

class CfnFlowDefSourcepropertyParams(pydantic.BaseModel):
    decryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    description: typing.Optional[str] = pydantic.Field(None, description='')
    entitlement_arn: typing.Optional[str] = pydantic.Field(None, description='')
    gateway_bridge_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_GatewayBridgeSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ingest_ip: typing.Optional[str] = pydantic.Field(None, description='')
    ingest_port: typing.Union[int, float, None] = pydantic.Field(None, description='')
    max_bitrate: typing.Union[int, float, None] = pydantic.Field(None, description='')
    max_latency: typing.Union[int, float, None] = pydantic.Field(None, description='')
    max_sync_buffer: typing.Union[int, float, None] = pydantic.Field(None, description='')
    media_stream_source_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MediaStreamSourceConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='')
    min_latency: typing.Union[int, float, None] = pydantic.Field(None, description='')
    name: typing.Optional[str] = pydantic.Field(None, description='')
    protocol: typing.Optional[str] = pydantic.Field(None, description='')
    sender_control_port: typing.Union[int, float, None] = pydantic.Field(None, description='')
    sender_ip_address: typing.Optional[str] = pydantic.Field(None, description='')
    source_arn: typing.Optional[str] = pydantic.Field(None, description='')
    source_ingest_port: typing.Optional[str] = pydantic.Field(None, description='')
    source_listener_address: typing.Optional[str] = pydantic.Field(None, description='')
    source_listener_port: typing.Union[int, float, None] = pydantic.Field(None, description='')
    stream_id: typing.Optional[str] = pydantic.Field(None, description='')
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='')
    whitelist_cidr: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefVpcinterfaceattachmentpropertyParams(pydantic.BaseModel):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefVpcinterfacepropertyParams(pydantic.BaseModel):
    name: str = pydantic.Field(..., description='')
    role_arn: str = pydantic.Field(..., description='')
    security_group_ids: typing.Sequence[str] = pydantic.Field(..., description='')
    subnet_id: str = pydantic.Field(..., description='')
    network_interface_ids: typing.Optional[typing.Sequence[str]] = pydantic.Field(None, description='')
    network_interface_type: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnFlowDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnFlowDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnFlowDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnFlowDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnFlowDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnFlowDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnFlowDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnFlowDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnFlowDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowEntitlement
class CfnFlowEntitlementDef(BaseCfnResource):
    description: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A description of the entitlement. This description appears only on the MediaConnect console and is not visible outside of the current AWS account.\n')
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the flow.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the entitlement. This value must be unique within the current flow.\n')
    subscribers: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The AWS account IDs that you want to share your content with. The receiving accounts (subscribers) will be allowed to create their own flows using your content as the source.\n')
    data_transfer_subscriber_fee_percent: typing.Union[int, float, None] = pydantic.Field(None, description='The percentage of the entitlement data transfer fee that you want the subscriber to be responsible for. Default: - 0\n')
    encryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowEntitlement_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The type of encryption that MediaConnect will use on the output that is associated with the entitlement.\n')
    entitlement_status: typing.Optional[str] = pydantic.Field(None, description='An indication of whether the new entitlement should be enabled or disabled as soon as it is created. If you don’t specify the entitlementStatus field in your request, MediaConnect sets it to ENABLED.')
    _init_params: typing.ClassVar[list[str]] = ['description', 'flow_arn', 'name', 'subscribers', 'data_transfer_subscriber_fee_percent', 'encryption', 'entitlement_status']
    _method_names: typing.ClassVar[list[str]] = ['EncryptionProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowEntitlement'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnFlowEntitlementDefConfig] = pydantic.Field(None)


class CfnFlowEntitlementDefConfig(pydantic.BaseModel):
    EncryptionProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefEncryptionpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowEntitlementDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnFlowEntitlementDefEncryptionpropertyParams(pydantic.BaseModel):
    algorithm: str = pydantic.Field(..., description='')
    role_arn: str = pydantic.Field(..., description='')
    constant_initialization_vector: typing.Optional[str] = pydantic.Field(None, description='')
    device_id: typing.Optional[str] = pydantic.Field(None, description='')
    key_type: typing.Optional[str] = pydantic.Field(None, description='')
    region: typing.Optional[str] = pydantic.Field(None, description='')
    resource_id: typing.Optional[str] = pydantic.Field(None, description='')
    secret_arn: typing.Optional[str] = pydantic.Field(None, description='')
    url: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowEntitlementDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnFlowEntitlementDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowEntitlementDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnFlowEntitlementDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowEntitlementDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnFlowEntitlementDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnFlowEntitlementDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnFlowEntitlementDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnFlowEntitlementDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnFlowEntitlementDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowEntitlementDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnFlowEntitlementDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnFlowEntitlementDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowEntitlementDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutput
class CfnFlowOutputDef(BaseCfnResource):
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the flow this output is attached to.\n')
    protocol: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The protocol to use for the output.\n')
    cidr_allow_list: typing.Optional[typing.Sequence[str]] = pydantic.Field(None, description='The range of IP addresses that are allowed to initiate output requests to this flow. Format the IP addresses as a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n')
    description: typing.Optional[str] = pydantic.Field(None, description='A description of the output. This description is not visible outside of the current AWS account even if the account grants entitlements to other accounts.\n')
    destination: typing.Optional[str] = pydantic.Field(None, description='The IP address where you want to send the output.\n')
    encryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The encryption credentials that you want to use for the output.\n')
    max_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum latency in milliseconds. This parameter applies only to RIST-based, Zixi-based, and Fujitsu-based streams.\n')
    media_stream_output_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_MediaStreamOutputConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The definition for each media stream that is associated with the output.\n')
    min_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The minimum latency in milliseconds for SRT-based streams. In streams that use the SRT protocol, this value that you set on your MediaConnect source or output represents the minimal potential latency of that connection. The latency of the stream is set to the highest number between the sender’s minimum latency and the receiver’s minimum latency.\n')
    name: typing.Optional[str] = pydantic.Field(None, description='The name of the output. This value must be unique within the current flow.\n')
    port: typing.Union[int, float, None] = pydantic.Field(None, description='The port to use when MediaConnect distributes content to the output.\n')
    remote_id: typing.Optional[str] = pydantic.Field(None, description='The identifier that is assigned to the Zixi receiver. This parameter applies only to outputs that use Zixi pull.\n')
    smoothing_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The smoothing latency in milliseconds for RIST, RTP, and RTP-FEC streams.\n')
    stream_id: typing.Optional[str] = pydantic.Field(None, description='The stream ID that you want to use for this transport. This parameter applies only to Zixi and SRT caller-based streams.\n')
    vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The VPC interface that you want to send your output to.')
    _init_params: typing.ClassVar[list[str]] = ['flow_arn', 'protocol', 'cidr_allow_list', 'description', 'destination', 'encryption', 'max_latency', 'media_stream_output_configurations', 'min_latency', 'name', 'port', 'remote_id', 'smoothing_latency', 'stream_id', 'vpc_interface_attachment']
    _method_names: typing.ClassVar[list[str]] = ['DestinationConfigurationProperty', 'EncodingParametersProperty', 'EncryptionProperty', 'InterfaceProperty', 'MediaStreamOutputConfigurationProperty', 'VpcInterfaceAttachmentProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutput'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnFlowOutputDefConfig] = pydantic.Field(None)


class CfnFlowOutputDefConfig(pydantic.BaseModel):
    DestinationConfigurationProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefDestinationconfigurationpropertyParams]] = pydantic.Field(None, description='')
    EncodingParametersProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefEncodingparameterspropertyParams]] = pydantic.Field(None, description='')
    EncryptionProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefEncryptionpropertyParams]] = pydantic.Field(None, description='')
    InterfaceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefInterfacepropertyParams]] = pydantic.Field(None, description='')
    MediaStreamOutputConfigurationProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefMediastreamoutputconfigurationpropertyParams]] = pydantic.Field(None, description='')
    VpcInterfaceAttachmentProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefVpcinterfaceattachmentpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowOutputDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnFlowOutputDefDestinationconfigurationpropertyParams(pydantic.BaseModel):
    destination_ip: str = pydantic.Field(..., description='')
    destination_port: typing.Union[int, float] = pydantic.Field(..., description='')
    interface: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_InterfacePropertyDef, dict[str, typing.Any]] = pydantic.Field(..., description='')
    ...

class CfnFlowOutputDefEncodingparameterspropertyParams(pydantic.BaseModel):
    compression_factor: typing.Union[int, float] = pydantic.Field(..., description='')
    encoder_profile: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowOutputDefEncryptionpropertyParams(pydantic.BaseModel):
    role_arn: str = pydantic.Field(..., description='')
    secret_arn: str = pydantic.Field(..., description='')
    algorithm: typing.Optional[str] = pydantic.Field(None, description='')
    key_type: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowOutputDefInterfacepropertyParams(pydantic.BaseModel):
    name: str = pydantic.Field(..., description='')
    ...

class CfnFlowOutputDefMediastreamoutputconfigurationpropertyParams(pydantic.BaseModel):
    encoding_name: str = pydantic.Field(..., description='')
    media_stream_name: str = pydantic.Field(..., description='')
    destination_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_DestinationConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='')
    encoding_parameters: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_EncodingParametersPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnFlowOutputDefVpcinterfaceattachmentpropertyParams(pydantic.BaseModel):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowOutputDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnFlowOutputDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowOutputDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnFlowOutputDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowOutputDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnFlowOutputDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnFlowOutputDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnFlowOutputDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnFlowOutputDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnFlowOutputDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowOutputDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnFlowOutputDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnFlowOutputDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowOutputDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowSource
class CfnFlowSourceDef(BaseCfnResource):
    description: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A description of the source. This description is not visible outside of the current AWS account.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the source.\n')
    decryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowSource_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The type of encryption that is used on the content ingested from the source.\n')
    entitlement_arn: typing.Optional[str] = pydantic.Field(None, description="The ARN of the entitlement that allows you to subscribe to the flow. The entitlement is set by the content originator, and the ARN is generated as part of the originator's flow.\n")
    flow_arn: typing.Optional[str] = pydantic.Field(None, description='The Amazon Resource Name (ARN) of the flow this source is connected to. The flow must have Failover enabled to add an additional source.\n')
    gateway_bridge_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowSource_GatewayBridgeSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The source configuration for cloud flows receiving a stream from a bridge.\n')
    ingest_port: typing.Union[int, float, None] = pydantic.Field(None, description='The port that the flow listens on for incoming content. If the protocol of the source is Zixi, the port must be set to 2088.\n')
    max_bitrate: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum bitrate for RIST, RTP, and RTP-FEC streams.\n')
    max_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum latency in milliseconds. This parameter applies only to RIST-based, Zixi-based, and Fujitsu-based streams. Default: - 2000\n')
    min_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The minimum latency in milliseconds for SRT-based streams. In streams that use the SRT protocol, this value that you set on your MediaConnect source or output represents the minimal potential latency of that connection. The latency of the stream is set to the highest number between the sender’s minimum latency and the receiver’s minimum latency. Default: - 2000\n')
    protocol: typing.Optional[str] = pydantic.Field(None, description="The protocol that the source uses to deliver the content to MediaConnect. Adding additional sources to an existing flow requires Failover to be enabled. When you enable Failover, the additional source must use the same protocol as the existing source. Only the following protocols support failover: Zixi-push, RTP-FEC, RTP, RIST and SRT protocols. If you use failover with SRT caller or listener, the ``FailoverMode`` property must be set to ``FAILOVER`` . The ``FailoverMode`` property is found in the ``FailoverConfig`` resource of the same flow ARN you used for the source's ``FlowArn`` property. SRT caller/listener does not support merge mode failover.\n")
    sender_control_port: typing.Union[int, float, None] = pydantic.Field(None, description='The port that the flow uses to send outbound requests to initiate connection with the sender.\n')
    sender_ip_address: typing.Optional[str] = pydantic.Field(None, description='The IP address that the flow communicates with to initiate connection with the sender.\n')
    source_listener_address: typing.Optional[str] = pydantic.Field(None, description='Source IP or domain name for SRT-caller protocol.\n')
    source_listener_port: typing.Union[int, float, None] = pydantic.Field(None, description='Source port for SRT-caller protocol.\n')
    stream_id: typing.Optional[str] = pydantic.Field(None, description='The stream ID that you want to use for this transport. This parameter applies only to Zixi and SRT caller-based streams.\n')
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n')
    whitelist_cidr: typing.Optional[str] = pydantic.Field(None, description='The range of IP addresses that are allowed to contribute content to your source. Format the IP addresses as a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.')
    _init_params: typing.ClassVar[list[str]] = ['description', 'name', 'decryption', 'entitlement_arn', 'flow_arn', 'gateway_bridge_source', 'ingest_port', 'max_bitrate', 'max_latency', 'min_latency', 'protocol', 'sender_control_port', 'sender_ip_address', 'source_listener_address', 'source_listener_port', 'stream_id', 'vpc_interface_name', 'whitelist_cidr']
    _method_names: typing.ClassVar[list[str]] = ['EncryptionProperty', 'GatewayBridgeSourceProperty', 'VpcInterfaceAttachmentProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowSource'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnFlowSourceDefConfig] = pydantic.Field(None)


class CfnFlowSourceDefConfig(pydantic.BaseModel):
    EncryptionProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefEncryptionpropertyParams]] = pydantic.Field(None, description='')
    GatewayBridgeSourceProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefGatewaybridgesourcepropertyParams]] = pydantic.Field(None, description='')
    VpcInterfaceAttachmentProperty: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefVpcinterfaceattachmentpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowSourceDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnFlowSourceDefEncryptionpropertyParams(pydantic.BaseModel):
    role_arn: str = pydantic.Field(..., description='')
    algorithm: typing.Optional[str] = pydantic.Field(None, description='')
    constant_initialization_vector: typing.Optional[str] = pydantic.Field(None, description='')
    device_id: typing.Optional[str] = pydantic.Field(None, description='')
    key_type: typing.Optional[str] = pydantic.Field(None, description='')
    region: typing.Optional[str] = pydantic.Field(None, description='')
    resource_id: typing.Optional[str] = pydantic.Field(None, description='')
    secret_arn: typing.Optional[str] = pydantic.Field(None, description='')
    url: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowSourceDefGatewaybridgesourcepropertyParams(pydantic.BaseModel):
    bridge_arn: str = pydantic.Field(..., description='')
    vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowSource_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='')
    ...

class CfnFlowSourceDefVpcinterfaceattachmentpropertyParams(pydantic.BaseModel):
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='')
    ...

class CfnFlowSourceDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnFlowSourceDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowSourceDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnFlowSourceDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowSourceDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnFlowSourceDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnFlowSourceDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnFlowSourceDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnFlowSourceDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnFlowSourceDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowSourceDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnFlowSourceDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnFlowSourceDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowSourceDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowVpcInterface
class CfnFlowVpcInterfaceDef(BaseCfnResource):
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the flow.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the VPC Interface. This value must be unique within the current flow.\n')
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the role that you created when you set up MediaConnect as a trusted service.\n')
    security_group_ids: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The VPC security groups that you want MediaConnect to use for your VPC configuration. You must include at least one security group in the request.\n')
    subnet_id: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The subnet IDs that you want to use for your VPC interface. A range of IP addresses in your VPC. When you create your VPC, you specify a range of IPv4 addresses for the VPC in the form of a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16. This is the primary CIDR block for your VPC. When you create a subnet for your VPC, you specify the CIDR block for the subnet, which is a subset of the VPC CIDR block. The subnets that you use across all VPC interfaces on the flow must be in the same Availability Zone as the flow.')
    _init_params: typing.ClassVar[list[str]] = ['flow_arn', 'name', 'role_arn', 'security_group_ids', 'subnet_id']
    _method_names: typing.ClassVar[list[str]] = ['add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowVpcInterface'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnFlowVpcInterfaceDefConfig] = pydantic.Field(None)


class CfnFlowVpcInterfaceDefConfig(pydantic.BaseModel):
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnFlowVpcInterfaceDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnFlowVpcInterfaceDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnFlowVpcInterfaceDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowVpcInterfaceDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnFlowVpcInterfaceDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowVpcInterfaceDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnFlowVpcInterfaceDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnFlowVpcInterfaceDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnFlowVpcInterfaceDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnFlowVpcInterfaceDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnFlowVpcInterfaceDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnFlowVpcInterfaceDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnFlowVpcInterfaceDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnFlowVpcInterfaceDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnFlowVpcInterfaceDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnGateway
class CfnGatewayDef(BaseCfnResource):
    egress_cidr_blocks: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The range of IP addresses that are allowed to contribute content or initiate output requests for flows communicating with this gateway. These IP addresses should be in the form of a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the network. This name is used to reference the network and must be unique among networks in this gateway.\n')
    networks: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnGateway_GatewayNetworkPropertyDef, dict[str, typing.Any]]]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The list of networks that you want to add.')
    _init_params: typing.ClassVar[list[str]] = ['egress_cidr_blocks', 'name', 'networks']
    _method_names: typing.ClassVar[list[str]] = ['GatewayNetworkProperty', 'add_deletion_override', 'add_dependency', 'add_depends_on', 'add_metadata', 'add_override', 'add_property_deletion_override', 'add_property_override', 'apply_removal_policy', 'get_att', 'get_metadata', 'inspect', 'obtain_dependencies', 'obtain_resource_dependencies', 'override_logical_id', 'remove_dependency', 'replace_dependency']
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnGateway'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...


    resource_config: typing.Optional[models.aws_mediaconnect.CfnGatewayDefConfig] = pydantic.Field(None)


class CfnGatewayDefConfig(pydantic.BaseModel):
    GatewayNetworkProperty: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefGatewaynetworkpropertyParams]] = pydantic.Field(None, description='')
    add_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddDeletionOverrideParams]] = pydantic.Field(None, description='Syntactic sugar for ``addOverride(path, undefined)``.')
    add_dependency: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddDependencyParams]] = pydantic.Field(None, description='Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.\nThis can be used for resources across stacks (or nested stack) boundaries\nand the dependency will automatically be transferred to the relevant scope.')
    add_depends_on: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddDependsOnParams]] = pydantic.Field(None, description='(deprecated) Indicates that this resource depends on another resource and cannot be provisioned unless the other resource has been successfully provisioned.')
    add_metadata: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddMetadataParams]] = pydantic.Field(None, description='Add a value to the CloudFormation Resource Metadata.')
    add_override: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddOverrideParams]] = pydantic.Field(None, description='Adds an override to the synthesized CloudFormation resource.\nTo add a\nproperty override, either use ``addPropertyOverride`` or prefix ``path`` with\n"Properties." (i.e. ``Properties.TopicName``).\n\nIf the override is nested, separate each nested level using a dot (.) in the path parameter.\nIf there is an array as part of the nesting, specify the index in the path.\n\nTo include a literal ``.`` in the property name, prefix with a ``\\``. In most\nprogramming languages you will need to write this as ``"\\\\."`` because the\n``\\`` itself will need to be escaped.\n\nFor example::\n\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.0.Projection.NonKeyAttributes", ["myattribute"])\n   cfn_resource.add_override("Properties.GlobalSecondaryIndexes.1.ProjectionType", "INCLUDE")\n\nwould add the overrides Example::\n\n   "Properties": {\n     "GlobalSecondaryIndexes": [\n       {\n         "Projection": {\n           "NonKeyAttributes": [ "myattribute" ]\n           ...\n         }\n         ...\n       },\n       {\n         "ProjectionType": "INCLUDE"\n         ...\n       },\n     ]\n     ...\n   }\n\nThe ``value`` argument to ``addOverride`` will not be processed or translated\nin any way. Pass raw JSON values in here with the correct capitalization\nfor CloudFormation. If you pass CDK classes or structs, they will be\nrendered with lowercased key names, and CloudFormation will reject the\ntemplate.')
    add_property_deletion_override: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddPropertyDeletionOverrideParams]] = pydantic.Field(None, description='Adds an override that deletes the value of a property from the resource definition.')
    add_property_override: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefAddPropertyOverrideParams]] = pydantic.Field(None, description='Adds an override to a resource property.\nSyntactic sugar for ``addOverride("Properties.<...>", value)``.')
    apply_removal_policy: typing.Optional[list[models.GenericApplyRemovalPolicyParams]] = pydantic.Field(None)
    get_att: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefGetAttParams]] = pydantic.Field(None, description='Returns a token for an runtime attribute of this resource.\nIdeally, use generated attribute accessors (e.g. ``resource.arn``), but this can be used for future compatibility\nin case there is no generated attribute.')
    get_metadata: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefGetMetadataParams]] = pydantic.Field(None, description='Retrieve a value value from the CloudFormation Resource Metadata.')
    inspect: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefInspectParams]] = pydantic.Field(None, description='Examines the CloudFormation resource and discloses attributes.')
    obtain_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Retrieves an array of resources this resource depends on.\nThis assembles dependencies on resources across stacks (including nested stacks)\nautomatically.')
    obtain_resource_dependencies: typing.Optional[bool] = pydantic.Field(None, description='Get a shallow copy of dependencies between this resource and other resources in the same stack.')
    override_logical_id: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefOverrideLogicalIdParams]] = pydantic.Field(None, description='Overrides the auto-generated logical ID with a specific ID.')
    remove_dependency: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefRemoveDependencyParams]] = pydantic.Field(None, description='Indicates that this resource no longer depends on another resource.\nThis can be used for resources across stacks (including nested stacks)\nand the dependency will automatically be removed from the relevant scope.')
    replace_dependency: typing.Optional[list[models.aws_mediaconnect.CfnGatewayDefReplaceDependencyParams]] = pydantic.Field(None, description='Replaces one dependency with another.')

class CfnGatewayDefGatewaynetworkpropertyParams(pydantic.BaseModel):
    cidr_block: str = pydantic.Field(..., description='')
    name: str = pydantic.Field(..., description='')
    ...

class CfnGatewayDefAddDeletionOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='The path of the value to delete.')
    ...

class CfnGatewayDefAddDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnGatewayDefAddDependsOnParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-\n\n:deprecated: use addDependency\n\n:stability: deprecated\n')
    ...

class CfnGatewayDefAddMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n')
    value: typing.Any = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnGatewayDefAddOverrideParams(pydantic.BaseModel):
    path: str = pydantic.Field(..., description='- The path of the property, you can use dot notation to override values in complex types. Any intermediate keys will be created as needed.\n')
    value: typing.Any = pydantic.Field(..., description='- The value. Could be primitive or complex.')
    ...

class CfnGatewayDefAddPropertyDeletionOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path to the property.')
    ...

class CfnGatewayDefAddPropertyOverrideParams(pydantic.BaseModel):
    property_path: str = pydantic.Field(..., description='The path of the property.\n')
    value: typing.Any = pydantic.Field(..., description='The value.')
    ...

class CfnGatewayDefApplyRemovalPolicyParams(pydantic.BaseModel):
    policy: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description='-\n')
    apply_to_update_replace_policy: typing.Optional[bool] = pydantic.Field(None, description='Apply the same deletion policy to the resource\'s "UpdateReplacePolicy". Default: true\n')
    default: typing.Optional[aws_cdk.RemovalPolicy] = pydantic.Field(None, description="The default policy to apply in case the removal policy is not defined. Default: - Default value is resource specific. To determine the default value for a resource, please consult that specific resource's documentation.\n\n:see: https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-attribute-deletionpolicy.html#aws-attribute-deletionpolicy-options\n")
    ...

class CfnGatewayDefGetAttParams(pydantic.BaseModel):
    attribute_name: str = pydantic.Field(..., description='The name of the attribute.\n')
    type_hint: typing.Optional[aws_cdk.ResolutionTypeHint] = pydantic.Field(None, description='-')
    return_config: typing.Optional[list[models.core.ReferenceDefConfig]] = pydantic.Field(None)
    ...

class CfnGatewayDefGetMetadataParams(pydantic.BaseModel):
    key: str = pydantic.Field(..., description='-\n\n:see:\n\nhttps://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/metadata-section-structure.html\n\nNote that this is a different set of metadata from CDK node metadata; this\nmetadata ends up in the stack template under the resource, whereas CDK\nnode metadata ends up in the Cloud Assembly.\n')
    ...

class CfnGatewayDefInspectParams(pydantic.BaseModel):
    inspector: models.TreeInspectorDef = pydantic.Field(..., description='tree inspector to collect and process attributes.')
    ...

class CfnGatewayDefOverrideLogicalIdParams(pydantic.BaseModel):
    new_logical_id: str = pydantic.Field(..., description='The new logical ID to use for this stack element.')
    ...

class CfnGatewayDefRemoveDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='-')
    ...

class CfnGatewayDefReplaceDependencyParams(pydantic.BaseModel):
    target: models.CfnResourceDef = pydantic.Field(..., description='The dependency to replace.\n')
    new_target: models.CfnResourceDef = pydantic.Field(..., description='The new dependency to add.')
    ...


#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeOutputProps
class CfnBridgeOutputPropsDef(BaseCfnProperty):
    bridge_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the bridge that you want to describe.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The network output name. This name is used to reference the output and must be unique among outputs in this bridge.\n')
    network_output: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeOutput_BridgeNetworkOutputPropertyDef, dict[str, typing.Any]] = pydantic.Field(REQUIRED_INIT_PARAM, description='Add a network output to an existing bridge.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-bridgeoutput.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_bridge_output_props = mediaconnect.CfnBridgeOutputProps(\n        bridge_arn="bridgeArn",\n        name="name",\n        network_output=mediaconnect.CfnBridgeOutput.BridgeNetworkOutputProperty(\n            ip_address="ipAddress",\n            network_name="networkName",\n            port=123,\n            protocol="protocol",\n            ttl=123\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['bridge_arn', 'name', 'network_output']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeOutputProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeProps
class CfnBridgePropsDef(BaseCfnProperty):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the bridge. This name can not be modified after the bridge is created.\n')
    placement_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The bridge placement Amazon Resource Number (ARN).\n')
    sources: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeSourcePropertyDef, dict[str, typing.Any]]]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The sources that you want to add to this bridge.\n')
    egress_gateway_bridge: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_EgressGatewayBridgePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Create a bridge with the egress bridge type. An egress bridge is a cloud-to-ground bridge. The content comes from an existing MediaConnect flow and is delivered to your premises.\n')
    ingress_gateway_bridge: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_IngressGatewayBridgePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Create a bridge with the ingress bridge type. An ingress bridge is a ground-to-cloud bridge. The content originates at your premises and is delivered to the cloud.\n')
    outputs: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_BridgeOutputPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The outputs that you want to add to this bridge.\n')
    source_failover_config: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridge_FailoverConfigPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The settings for source failover.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-bridge.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_bridge_props = mediaconnect.CfnBridgeProps(\n        name="name",\n        placement_arn="placementArn",\n        sources=[mediaconnect.CfnBridge.BridgeSourceProperty(\n            flow_source=mediaconnect.CfnBridge.BridgeFlowSourceProperty(\n                flow_arn="flowArn",\n                name="name",\n\n                # the properties below are optional\n                flow_vpc_interface_attachment=mediaconnect.CfnBridge.VpcInterfaceAttachmentProperty(\n                    vpc_interface_name="vpcInterfaceName"\n                )\n            ),\n            network_source=mediaconnect.CfnBridge.BridgeNetworkSourceProperty(\n                multicast_ip="multicastIp",\n                name="name",\n                network_name="networkName",\n                port=123,\n                protocol="protocol"\n            )\n        )],\n\n        # the properties below are optional\n        egress_gateway_bridge=mediaconnect.CfnBridge.EgressGatewayBridgeProperty(\n            max_bitrate=123\n        ),\n        ingress_gateway_bridge=mediaconnect.CfnBridge.IngressGatewayBridgeProperty(\n            max_bitrate=123,\n            max_outputs=123\n        ),\n        outputs=[mediaconnect.CfnBridge.BridgeOutputProperty(\n            network_output=mediaconnect.CfnBridge.BridgeNetworkOutputProperty(\n                ip_address="ipAddress",\n                name="name",\n                network_name="networkName",\n                port=123,\n                protocol="protocol",\n                ttl=123\n            )\n        )],\n        source_failover_config=mediaconnect.CfnBridge.FailoverConfigProperty(\n            failover_mode="failoverMode",\n\n            # the properties below are optional\n            source_priority=mediaconnect.CfnBridge.SourcePriorityProperty(\n                primary_source="primarySource"\n            ),\n            state="state"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['name', 'placement_arn', 'sources', 'egress_gateway_bridge', 'ingress_gateway_bridge', 'outputs', 'source_failover_config']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnBridgeSourceProps
class CfnBridgeSourcePropsDef(BaseCfnProperty):
    bridge_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The ARN of the bridge that you want to describe.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the flow source. This name is used to reference the source and must be unique among sources in this bridge.\n')
    flow_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeSource_BridgeFlowSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Add a flow source to an existing bridge.\n')
    network_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnBridgeSource_BridgeNetworkSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Add a network source to an existing bridge.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-bridgesource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_bridge_source_props = mediaconnect.CfnBridgeSourceProps(\n        bridge_arn="bridgeArn",\n        name="name",\n\n        # the properties below are optional\n        flow_source=mediaconnect.CfnBridgeSource.BridgeFlowSourceProperty(\n            flow_arn="flowArn",\n\n            # the properties below are optional\n            flow_vpc_interface_attachment=mediaconnect.CfnBridgeSource.VpcInterfaceAttachmentProperty(\n                vpc_interface_name="vpcInterfaceName"\n            )\n        ),\n        network_source=mediaconnect.CfnBridgeSource.BridgeNetworkSourceProperty(\n            multicast_ip="multicastIp",\n            network_name="networkName",\n            port=123,\n            protocol="protocol"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['bridge_arn', 'name', 'flow_source', 'network_source']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnBridgeSourceProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowEntitlementProps
class CfnFlowEntitlementPropsDef(BaseCfnProperty):
    description: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A description of the entitlement. This description appears only on the MediaConnect console and is not visible outside of the current AWS account.\n')
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the flow.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the entitlement. This value must be unique within the current flow.\n')
    subscribers: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The AWS account IDs that you want to share your content with. The receiving accounts (subscribers) will be allowed to create their own flows using your content as the source.\n')
    data_transfer_subscriber_fee_percent: typing.Union[int, float, None] = pydantic.Field(None, description='The percentage of the entitlement data transfer fee that you want the subscriber to be responsible for. Default: - 0\n')
    encryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowEntitlement_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The type of encryption that MediaConnect will use on the output that is associated with the entitlement.\n')
    entitlement_status: typing.Optional[str] = pydantic.Field(None, description='An indication of whether the new entitlement should be enabled or disabled as soon as it is created. If you don’t specify the entitlementStatus field in your request, MediaConnect sets it to ENABLED.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-flowentitlement.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_flow_entitlement_props = mediaconnect.CfnFlowEntitlementProps(\n        description="description",\n        flow_arn="flowArn",\n        name="name",\n        subscribers=["subscribers"],\n\n        # the properties below are optional\n        data_transfer_subscriber_fee_percent=123,\n        encryption=mediaconnect.CfnFlowEntitlement.EncryptionProperty(\n            algorithm="algorithm",\n            role_arn="roleArn",\n\n            # the properties below are optional\n            constant_initialization_vector="constantInitializationVector",\n            device_id="deviceId",\n            key_type="keyType",\n            region="region",\n            resource_id="resourceId",\n            secret_arn="secretArn",\n            url="url"\n        ),\n        entitlement_status="entitlementStatus"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['description', 'flow_arn', 'name', 'subscribers', 'data_transfer_subscriber_fee_percent', 'encryption', 'entitlement_status']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowEntitlementProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowOutputProps
class CfnFlowOutputPropsDef(BaseCfnProperty):
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the flow this output is attached to.\n')
    protocol: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The protocol to use for the output.\n')
    cidr_allow_list: typing.Optional[typing.Sequence[str]] = pydantic.Field(None, description='The range of IP addresses that are allowed to initiate output requests to this flow. Format the IP addresses as a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n')
    description: typing.Optional[str] = pydantic.Field(None, description='A description of the output. This description is not visible outside of the current AWS account even if the account grants entitlements to other accounts.\n')
    destination: typing.Optional[str] = pydantic.Field(None, description='The IP address where you want to send the output.\n')
    encryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The encryption credentials that you want to use for the output.\n')
    max_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum latency in milliseconds. This parameter applies only to RIST-based, Zixi-based, and Fujitsu-based streams.\n')
    media_stream_output_configurations: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_MediaStreamOutputConfigurationPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The definition for each media stream that is associated with the output.\n')
    min_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The minimum latency in milliseconds for SRT-based streams. In streams that use the SRT protocol, this value that you set on your MediaConnect source or output represents the minimal potential latency of that connection. The latency of the stream is set to the highest number between the sender’s minimum latency and the receiver’s minimum latency.\n')
    name: typing.Optional[str] = pydantic.Field(None, description='The name of the output. This value must be unique within the current flow.\n')
    port: typing.Union[int, float, None] = pydantic.Field(None, description='The port to use when MediaConnect distributes content to the output.\n')
    remote_id: typing.Optional[str] = pydantic.Field(None, description='The identifier that is assigned to the Zixi receiver. This parameter applies only to outputs that use Zixi pull.\n')
    smoothing_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The smoothing latency in milliseconds for RIST, RTP, and RTP-FEC streams.\n')
    stream_id: typing.Optional[str] = pydantic.Field(None, description='The stream ID that you want to use for this transport. This parameter applies only to Zixi and SRT caller-based streams.\n')
    vpc_interface_attachment: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowOutput_VpcInterfaceAttachmentPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The VPC interface that you want to send your output to.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-flowoutput.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_flow_output_props = mediaconnect.CfnFlowOutputProps(\n        flow_arn="flowArn",\n        protocol="protocol",\n\n        # the properties below are optional\n        cidr_allow_list=["cidrAllowList"],\n        description="description",\n        destination="destination",\n        encryption=mediaconnect.CfnFlowOutput.EncryptionProperty(\n            role_arn="roleArn",\n            secret_arn="secretArn",\n\n            # the properties below are optional\n            algorithm="algorithm",\n            key_type="keyType"\n        ),\n        max_latency=123,\n        media_stream_output_configurations=[mediaconnect.CfnFlowOutput.MediaStreamOutputConfigurationProperty(\n            encoding_name="encodingName",\n            media_stream_name="mediaStreamName",\n\n            # the properties below are optional\n            destination_configurations=[mediaconnect.CfnFlowOutput.DestinationConfigurationProperty(\n                destination_ip="destinationIp",\n                destination_port=123,\n                interface=mediaconnect.CfnFlowOutput.InterfaceProperty(\n                    name="name"\n                )\n            )],\n            encoding_parameters=mediaconnect.CfnFlowOutput.EncodingParametersProperty(\n                compression_factor=123,\n\n                # the properties below are optional\n                encoder_profile="encoderProfile"\n            )\n        )],\n        min_latency=123,\n        name="name",\n        port=123,\n        remote_id="remoteId",\n        smoothing_latency=123,\n        stream_id="streamId",\n        vpc_interface_attachment=mediaconnect.CfnFlowOutput.VpcInterfaceAttachmentProperty(\n            vpc_interface_name="vpcInterfaceName"\n        )\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['flow_arn', 'protocol', 'cidr_allow_list', 'description', 'destination', 'encryption', 'max_latency', 'media_stream_output_configurations', 'min_latency', 'name', 'port', 'remote_id', 'smoothing_latency', 'stream_id', 'vpc_interface_attachment']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowOutputProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowProps
class CfnFlowPropsDef(BaseCfnProperty):
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the flow.\n')
    source: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_SourcePropertyDef, dict[str, typing.Any]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The settings for the source that you want to use for the new flow.\n')
    availability_zone: typing.Optional[str] = pydantic.Field(None, description='The Availability Zone that you want to create the flow in. These options are limited to the Availability Zones within the current AWS Region.\n')
    maintenance: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MaintenancePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The maintenance settings you want to use for the flow.\n')
    media_streams: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_MediaStreamPropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The media streams associated with the flow. You can associate any of these media streams with sources and outputs on the flow.\n')
    source_failover_config: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_FailoverConfigPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The settings for source failover.\n')
    vpc_interfaces: typing.Union[models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlow_VpcInterfacePropertyDef, dict[str, typing.Any]]], None] = pydantic.Field(None, description='The VPC interfaces that you added to this flow.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-flow.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_flow_props = mediaconnect.CfnFlowProps(\n        name="name",\n        source=mediaconnect.CfnFlow.SourceProperty(\n            decryption=mediaconnect.CfnFlow.EncryptionProperty(\n                role_arn="roleArn",\n\n                # the properties below are optional\n                algorithm="algorithm",\n                constant_initialization_vector="constantInitializationVector",\n                device_id="deviceId",\n                key_type="keyType",\n                region="region",\n                resource_id="resourceId",\n                secret_arn="secretArn",\n                url="url"\n            ),\n            description="description",\n            entitlement_arn="entitlementArn",\n            gateway_bridge_source=mediaconnect.CfnFlow.GatewayBridgeSourceProperty(\n                bridge_arn="bridgeArn",\n\n                # the properties below are optional\n                vpc_interface_attachment=mediaconnect.CfnFlow.VpcInterfaceAttachmentProperty(\n                    vpc_interface_name="vpcInterfaceName"\n                )\n            ),\n            ingest_ip="ingestIp",\n            ingest_port=123,\n            max_bitrate=123,\n            max_latency=123,\n            max_sync_buffer=123,\n            media_stream_source_configurations=[mediaconnect.CfnFlow.MediaStreamSourceConfigurationProperty(\n                encoding_name="encodingName",\n                media_stream_name="mediaStreamName",\n\n                # the properties below are optional\n                input_configurations=[mediaconnect.CfnFlow.InputConfigurationProperty(\n                    input_port=123,\n                    interface=mediaconnect.CfnFlow.InterfaceProperty(\n                        name="name"\n                    )\n                )]\n            )],\n            min_latency=123,\n            name="name",\n            protocol="protocol",\n            sender_control_port=123,\n            sender_ip_address="senderIpAddress",\n            source_arn="sourceArn",\n            source_ingest_port="sourceIngestPort",\n            source_listener_address="sourceListenerAddress",\n            source_listener_port=123,\n            stream_id="streamId",\n            vpc_interface_name="vpcInterfaceName",\n            whitelist_cidr="whitelistCidr"\n        ),\n\n        # the properties below are optional\n        availability_zone="availabilityZone",\n        maintenance=mediaconnect.CfnFlow.MaintenanceProperty(\n            maintenance_day="maintenanceDay",\n            maintenance_start_hour="maintenanceStartHour"\n        ),\n        media_streams=[mediaconnect.CfnFlow.MediaStreamProperty(\n            media_stream_id=123,\n            media_stream_name="mediaStreamName",\n            media_stream_type="mediaStreamType",\n\n            # the properties below are optional\n            attributes=mediaconnect.CfnFlow.MediaStreamAttributesProperty(\n                fmtp=mediaconnect.CfnFlow.FmtpProperty(\n                    channel_order="channelOrder",\n                    colorimetry="colorimetry",\n                    exact_framerate="exactFramerate",\n                    par="par",\n                    range="range",\n                    scan_mode="scanMode",\n                    tcs="tcs"\n                ),\n                lang="lang"\n            ),\n            clock_rate=123,\n            description="description",\n            fmt=123,\n            video_format="videoFormat"\n        )],\n        source_failover_config=mediaconnect.CfnFlow.FailoverConfigProperty(\n            failover_mode="failoverMode",\n            recovery_window=123,\n            source_priority=mediaconnect.CfnFlow.SourcePriorityProperty(\n                primary_source="primarySource"\n            ),\n            state="state"\n        ),\n        vpc_interfaces=[mediaconnect.CfnFlow.VpcInterfaceProperty(\n            name="name",\n            role_arn="roleArn",\n            security_group_ids=["securityGroupIds"],\n            subnet_id="subnetId",\n\n            # the properties below are optional\n            network_interface_ids=["networkInterfaceIds"],\n            network_interface_type="networkInterfaceType"\n        )]\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['name', 'source', 'availability_zone', 'maintenance', 'media_streams', 'source_failover_config', 'vpc_interfaces']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowSourceProps
class CfnFlowSourcePropsDef(BaseCfnProperty):
    description: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='A description of the source. This description is not visible outside of the current AWS account.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the source.\n')
    decryption: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowSource_EncryptionPropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The type of encryption that is used on the content ingested from the source.\n')
    entitlement_arn: typing.Optional[str] = pydantic.Field(None, description="The ARN of the entitlement that allows you to subscribe to the flow. The entitlement is set by the content originator, and the ARN is generated as part of the originator's flow.\n")
    flow_arn: typing.Optional[str] = pydantic.Field(None, description='The Amazon Resource Name (ARN) of the flow this source is connected to. The flow must have Failover enabled to add an additional source.\n')
    gateway_bridge_source: typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnFlowSource_GatewayBridgeSourcePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='The source configuration for cloud flows receiving a stream from a bridge.\n')
    ingest_port: typing.Union[int, float, None] = pydantic.Field(None, description='The port that the flow listens on for incoming content. If the protocol of the source is Zixi, the port must be set to 2088.\n')
    max_bitrate: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum bitrate for RIST, RTP, and RTP-FEC streams.\n')
    max_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The maximum latency in milliseconds. This parameter applies only to RIST-based, Zixi-based, and Fujitsu-based streams. Default: - 2000\n')
    min_latency: typing.Union[int, float, None] = pydantic.Field(None, description='The minimum latency in milliseconds for SRT-based streams. In streams that use the SRT protocol, this value that you set on your MediaConnect source or output represents the minimal potential latency of that connection. The latency of the stream is set to the highest number between the sender’s minimum latency and the receiver’s minimum latency. Default: - 2000\n')
    protocol: typing.Optional[str] = pydantic.Field(None, description="The protocol that the source uses to deliver the content to MediaConnect. Adding additional sources to an existing flow requires Failover to be enabled. When you enable Failover, the additional source must use the same protocol as the existing source. Only the following protocols support failover: Zixi-push, RTP-FEC, RTP, RIST and SRT protocols. If you use failover with SRT caller or listener, the ``FailoverMode`` property must be set to ``FAILOVER`` . The ``FailoverMode`` property is found in the ``FailoverConfig`` resource of the same flow ARN you used for the source's ``FlowArn`` property. SRT caller/listener does not support merge mode failover.\n")
    sender_control_port: typing.Union[int, float, None] = pydantic.Field(None, description='The port that the flow uses to send outbound requests to initiate connection with the sender.\n')
    sender_ip_address: typing.Optional[str] = pydantic.Field(None, description='The IP address that the flow communicates with to initiate connection with the sender.\n')
    source_listener_address: typing.Optional[str] = pydantic.Field(None, description='Source IP or domain name for SRT-caller protocol.\n')
    source_listener_port: typing.Union[int, float, None] = pydantic.Field(None, description='Source port for SRT-caller protocol.\n')
    stream_id: typing.Optional[str] = pydantic.Field(None, description='The stream ID that you want to use for this transport. This parameter applies only to Zixi and SRT caller-based streams.\n')
    vpc_interface_name: typing.Optional[str] = pydantic.Field(None, description='The name of the VPC interface that you want to send your output to.\n')
    whitelist_cidr: typing.Optional[str] = pydantic.Field(None, description='The range of IP addresses that are allowed to contribute content to your source. Format the IP addresses as a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-flowsource.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_flow_source_props = mediaconnect.CfnFlowSourceProps(\n        description="description",\n        name="name",\n\n        # the properties below are optional\n        decryption=mediaconnect.CfnFlowSource.EncryptionProperty(\n            role_arn="roleArn",\n\n            # the properties below are optional\n            algorithm="algorithm",\n            constant_initialization_vector="constantInitializationVector",\n            device_id="deviceId",\n            key_type="keyType",\n            region="region",\n            resource_id="resourceId",\n            secret_arn="secretArn",\n            url="url"\n        ),\n        entitlement_arn="entitlementArn",\n        flow_arn="flowArn",\n        gateway_bridge_source=mediaconnect.CfnFlowSource.GatewayBridgeSourceProperty(\n            bridge_arn="bridgeArn",\n\n            # the properties below are optional\n            vpc_interface_attachment=mediaconnect.CfnFlowSource.VpcInterfaceAttachmentProperty(\n                vpc_interface_name="vpcInterfaceName"\n            )\n        ),\n        ingest_port=123,\n        max_bitrate=123,\n        max_latency=123,\n        min_latency=123,\n        protocol="protocol",\n        sender_control_port=123,\n        sender_ip_address="senderIpAddress",\n        source_listener_address="sourceListenerAddress",\n        source_listener_port=123,\n        stream_id="streamId",\n        vpc_interface_name="vpcInterfaceName",\n        whitelist_cidr="whitelistCidr"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['description', 'name', 'decryption', 'entitlement_arn', 'flow_arn', 'gateway_bridge_source', 'ingest_port', 'max_bitrate', 'max_latency', 'min_latency', 'protocol', 'sender_control_port', 'sender_ip_address', 'source_listener_address', 'source_listener_port', 'stream_id', 'vpc_interface_name', 'whitelist_cidr']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowSourceProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnFlowVpcInterfaceProps
class CfnFlowVpcInterfacePropsDef(BaseCfnProperty):
    flow_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the flow.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the VPC Interface. This value must be unique within the current flow.\n')
    role_arn: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The Amazon Resource Name (ARN) of the role that you created when you set up MediaConnect as a trusted service.\n')
    security_group_ids: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The VPC security groups that you want MediaConnect to use for your VPC configuration. You must include at least one security group in the request.\n')
    subnet_id: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The subnet IDs that you want to use for your VPC interface. A range of IP addresses in your VPC. When you create your VPC, you specify a range of IPv4 addresses for the VPC in the form of a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16. This is the primary CIDR block for your VPC. When you create a subnet for your VPC, you specify the CIDR block for the subnet, which is a subset of the VPC CIDR block. The subnets that you use across all VPC interfaces on the flow must be in the same Availability Zone as the flow.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-flowvpcinterface.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_flow_vpc_interface_props = mediaconnect.CfnFlowVpcInterfaceProps(\n        flow_arn="flowArn",\n        name="name",\n        role_arn="roleArn",\n        security_group_ids=["securityGroupIds"],\n        subnet_id="subnetId"\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['flow_arn', 'name', 'role_arn', 'security_group_ids', 'subnet_id']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnFlowVpcInterfaceProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




#  autogenerated from aws_cdk.aws_mediaconnect.CfnGatewayProps
class CfnGatewayPropsDef(BaseCfnProperty):
    egress_cidr_blocks: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The range of IP addresses that are allowed to contribute content or initiate output requests for flows communicating with this gateway. These IP addresses should be in the form of a Classless Inter-Domain Routing (CIDR) block; for example, 10.0.0.0/16.\n')
    name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of the network. This name is used to reference the network and must be unique among networks in this gateway.\n')
    networks: typing.Union[_REQUIRED_INIT_PARAM, models.UnsupportedResource, typing.Sequence[typing.Union[models.UnsupportedResource, models.aws_mediaconnect.CfnGateway_GatewayNetworkPropertyDef, dict[str, typing.Any]]]] = pydantic.Field(REQUIRED_INIT_PARAM, description='The list of networks that you want to add.\n\n:see: http://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-resource-mediaconnect-gateway.html\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import aws_mediaconnect as mediaconnect\n\n    cfn_gateway_props = mediaconnect.CfnGatewayProps(\n        egress_cidr_blocks=["egressCidrBlocks"],\n        name="name",\n        networks=[mediaconnect.CfnGateway.GatewayNetworkProperty(\n            cidr_block="cidrBlock",\n            name="name"\n        )]\n    )\n')
    _init_params: typing.ClassVar[list[str]] = ['egress_cidr_blocks', 'name', 'networks']
    _method_names: typing.ClassVar[list[str]] = []
    _classmethod_names: typing.ClassVar[list[str]] = []
    _cdk_class_fqn: typing.ClassVar[str] = 'aws_cdk.aws_mediaconnect.CfnGatewayProps'
    _alternate_constructor_method_names: typing.ClassVar[list[str]] = []
    ...




class ModuleModel(pydantic.BaseModel):
    CfnBridge_BridgeFlowSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_BridgeFlowSourcePropertyDef]] = pydantic.Field(None)
    CfnBridge_BridgeNetworkOutputProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_BridgeNetworkOutputPropertyDef]] = pydantic.Field(None)
    CfnBridge_BridgeNetworkSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_BridgeNetworkSourcePropertyDef]] = pydantic.Field(None)
    CfnBridge_BridgeOutputProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_BridgeOutputPropertyDef]] = pydantic.Field(None)
    CfnBridge_BridgeSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_BridgeSourcePropertyDef]] = pydantic.Field(None)
    CfnBridge_EgressGatewayBridgeProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_EgressGatewayBridgePropertyDef]] = pydantic.Field(None)
    CfnBridge_FailoverConfigProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_FailoverConfigPropertyDef]] = pydantic.Field(None)
    CfnBridge_IngressGatewayBridgeProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_IngressGatewayBridgePropertyDef]] = pydantic.Field(None)
    CfnBridge_SourcePriorityProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_SourcePriorityPropertyDef]] = pydantic.Field(None)
    CfnBridge_VpcInterfaceAttachmentProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridge_VpcInterfaceAttachmentPropertyDef]] = pydantic.Field(None)
    CfnBridgeOutput_BridgeNetworkOutputProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeOutput_BridgeNetworkOutputPropertyDef]] = pydantic.Field(None)
    CfnBridgeSource_BridgeFlowSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeSource_BridgeFlowSourcePropertyDef]] = pydantic.Field(None)
    CfnBridgeSource_BridgeNetworkSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeSource_BridgeNetworkSourcePropertyDef]] = pydantic.Field(None)
    CfnBridgeSource_VpcInterfaceAttachmentProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeSource_VpcInterfaceAttachmentPropertyDef]] = pydantic.Field(None)
    CfnFlow_EncryptionProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_EncryptionPropertyDef]] = pydantic.Field(None)
    CfnFlow_FailoverConfigProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_FailoverConfigPropertyDef]] = pydantic.Field(None)
    CfnFlow_FmtpProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_FmtpPropertyDef]] = pydantic.Field(None)
    CfnFlow_GatewayBridgeSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_GatewayBridgeSourcePropertyDef]] = pydantic.Field(None)
    CfnFlow_InputConfigurationProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_InputConfigurationPropertyDef]] = pydantic.Field(None)
    CfnFlow_InterfaceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_InterfacePropertyDef]] = pydantic.Field(None)
    CfnFlow_MaintenanceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_MaintenancePropertyDef]] = pydantic.Field(None)
    CfnFlow_MediaStreamAttributesProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_MediaStreamAttributesPropertyDef]] = pydantic.Field(None)
    CfnFlow_MediaStreamProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_MediaStreamPropertyDef]] = pydantic.Field(None)
    CfnFlow_MediaStreamSourceConfigurationProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_MediaStreamSourceConfigurationPropertyDef]] = pydantic.Field(None)
    CfnFlow_SourcePriorityProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_SourcePriorityPropertyDef]] = pydantic.Field(None)
    CfnFlow_SourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_SourcePropertyDef]] = pydantic.Field(None)
    CfnFlow_VpcInterfaceAttachmentProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_VpcInterfaceAttachmentPropertyDef]] = pydantic.Field(None)
    CfnFlow_VpcInterfaceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlow_VpcInterfacePropertyDef]] = pydantic.Field(None)
    CfnFlowEntitlement_EncryptionProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowEntitlement_EncryptionPropertyDef]] = pydantic.Field(None)
    CfnFlowOutput_DestinationConfigurationProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutput_DestinationConfigurationPropertyDef]] = pydantic.Field(None)
    CfnFlowOutput_EncodingParametersProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutput_EncodingParametersPropertyDef]] = pydantic.Field(None)
    CfnFlowOutput_EncryptionProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutput_EncryptionPropertyDef]] = pydantic.Field(None)
    CfnFlowOutput_InterfaceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutput_InterfacePropertyDef]] = pydantic.Field(None)
    CfnFlowOutput_MediaStreamOutputConfigurationProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutput_MediaStreamOutputConfigurationPropertyDef]] = pydantic.Field(None)
    CfnFlowOutput_VpcInterfaceAttachmentProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutput_VpcInterfaceAttachmentPropertyDef]] = pydantic.Field(None)
    CfnFlowSource_EncryptionProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowSource_EncryptionPropertyDef]] = pydantic.Field(None)
    CfnFlowSource_GatewayBridgeSourceProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowSource_GatewayBridgeSourcePropertyDef]] = pydantic.Field(None)
    CfnFlowSource_VpcInterfaceAttachmentProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowSource_VpcInterfaceAttachmentPropertyDef]] = pydantic.Field(None)
    CfnGateway_GatewayNetworkProperty: typing.Optional[dict[str, models.aws_mediaconnect.CfnGateway_GatewayNetworkPropertyDef]] = pydantic.Field(None)
    CfnBridge: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeDef]] = pydantic.Field(None)
    CfnBridgeOutput: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeOutputDef]] = pydantic.Field(None)
    CfnBridgeSource: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeSourceDef]] = pydantic.Field(None)
    CfnFlow: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowDef]] = pydantic.Field(None)
    CfnFlowEntitlement: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowEntitlementDef]] = pydantic.Field(None)
    CfnFlowOutput: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutputDef]] = pydantic.Field(None)
    CfnFlowSource: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowSourceDef]] = pydantic.Field(None)
    CfnFlowVpcInterface: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowVpcInterfaceDef]] = pydantic.Field(None)
    CfnGateway: typing.Optional[dict[str, models.aws_mediaconnect.CfnGatewayDef]] = pydantic.Field(None)
    CfnBridgeOutputProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeOutputPropsDef]] = pydantic.Field(None)
    CfnBridgeProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgePropsDef]] = pydantic.Field(None)
    CfnBridgeSourceProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnBridgeSourcePropsDef]] = pydantic.Field(None)
    CfnFlowEntitlementProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowEntitlementPropsDef]] = pydantic.Field(None)
    CfnFlowOutputProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowOutputPropsDef]] = pydantic.Field(None)
    CfnFlowProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowPropsDef]] = pydantic.Field(None)
    CfnFlowSourceProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowSourcePropsDef]] = pydantic.Field(None)
    CfnFlowVpcInterfaceProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnFlowVpcInterfacePropsDef]] = pydantic.Field(None)
    CfnGatewayProps: typing.Optional[dict[str, models.aws_mediaconnect.CfnGatewayPropsDef]] = pydantic.Field(None)
    ...

import models
