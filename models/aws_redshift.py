...
class CfnCluster_LoggingPropertiesPropertyDef(BaseStruct):
    bucket_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The name of an existing S3 bucket where the log files are to be stored. Constraints: - Must be in the same region as the cluster - The cluster must have read bucket and put object permissions\n')
...
class CfnClusterDef(BaseCfnResource):
    ...
    master_user_password: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The password associated with the admin user account for the cluster that is being created. You can\'t use ``MasterUserPassword`` if ``ManageMasterPassword`` is ``true`` . Constraints: - Must be between 8 and 64 characters in length. - Must contain at least one uppercase letter. - Must contain at least one lowercase letter. - Must contain one number. - Can be any printable ASCII character (ASCII code 33-126) except ``\'`` (single quote), ``"`` (double quote), ``\\`` , ``/`` , or ``@`` .\n')
...
class CfnScheduledActionDef(BaseCfnResource):
    ...
    target_action: typing.Union[models.UnsupportedResource, models.aws_redshift.CfnScheduledAction_ScheduledActionTypePropertyDef, dict[str, typing.Any], None] = pydantic.Field(None, description='A JSON format string of the Amazon Redshift API operation with input parameters. " ``{\\"ResizeCluster\\":{\\"NodeType\\":\\"ds2.8xlarge\\",\\"ClusterIdentifier\\":\\"my-test-cluster\\",\\"NumberOfNodes\\":3}}`` ".')
