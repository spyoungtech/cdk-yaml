...
class TriggerDef(BaseConstruct):
    handler: models.aws_lambda.FunctionDef = pydantic.Field(..., description='The AWS Lambda function of the handler to execute.\n')
...
class TriggerFunctionDef(BaseConstruct):
    ...
    _init_params: typing.ClassVar[list[str]] = ['code', 'handler', 'runtime', 'execute_after', 'execute_before', 'execute_on_handler_change', 'adot_instrumentation', 'allow_all_outbound', 'allow_public_subnet', 'architecture', 'code_signing_config', 'current_version_options', 'dead_letter_queue', 'dead_letter_queue_enabled', 'dead_letter_topic', 'description', 'environment', 'environment_encryption', 'ephemeral_storage_size', 'events', 'filesystem', 'function_name', 'initial_policy', 'insights_version', 'layers', 'log_retention', 'log_retention_retry_options', 'log_retention_role', 'memory_size', 'profiling', 'profiling_group', 'reserved_concurrent_executions', 'role', 'runtime_management_mode', 'security_groups', 'timeout', 'tracing', 'vpc', 'vpc_subnets', 'max_event_age', 'on_failure', 'on_success', 'retry_attempts']
    _method_names: typing.ClassVar[list[str]] = ['add_alias', 'add_environment', 'add_event_source', 'add_event_source_mapping', 'add_function_url', 'add_layers', 'add_permission', 'add_to_role_policy', 'apply_removal_policy', 'configure_async_invoke', 'consider_warning_on_invoke_function_permissions', 'execute_after', 'execute_before', 'grant_invoke', 'grant_invoke_url', 'invalidate_version_based_on', 'metric', 'metric_duration', 'metric_errors', 'metric_invocations', 'metric_throttles']
