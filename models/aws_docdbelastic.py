...
class CfnClusterDef(BaseCfnResource):
    admin_user_name: str = pydantic.Field(..., description='The name of the Amazon DocumentDB elastic clusters administrator. *Constraints* : - Must be from 1 to 63 letters or numbers. - The first character must be a letter. - Cannot be a reserved word.\n')
    auth_type: str = pydantic.Field(..., description='The authentication type used to determine where to fetch the password used for accessing the elastic cluster. Valid types are ``PLAIN_TEXT`` or ``SECRET_ARN`` .\n')
    cluster_name: str = pydantic.Field(..., description='The name of the new elastic cluster. This parameter is stored as a lowercase string. *Constraints* : - Must contain from 1 to 63 letters, numbers, or hyphens. - The first character must be a letter. - Cannot end with a hyphen or contain two consecutive hyphens. *Example* : ``my-cluster``\n')
    shard_capacity: typing.Union[int, float] = pydantic.Field(..., description='The number of vCPUs assigned to each elastic cluster shard. Maximum is 64. Allowed values are 2, 4, 8, 16, 32, 64.\n')
    shard_count: typing.Union[int, float] = pydantic.Field(..., description='The number of shards assigned to the elastic cluster. Maximum is 32.\n')
    ...
    _init_params: typing.ClassVar[list[str]] = ['admin_user_name', 'auth_type', 'cluster_name', 'shard_capacity', 'shard_count', 'admin_user_password', 'kms_key_id', 'preferred_maintenance_window', 'subnet_ids', 'tags', 'vpc_security_group_ids']
