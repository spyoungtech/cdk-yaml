...
class CodeBuildStepDef(BaseClass):
    ...
    commands: typing.Union[typing.Sequence[str], _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Commands to run.\n')
...
class CodePipelineSourceDefConnectionParams(pydantic.BaseModel):
    repo_string: str = pydantic.Field(..., description="A string that encodes owner and repository separated by a slash (e.g. 'owner/repo'). The provided string must be resolvable at runtime.\n")
    branch: str = pydantic.Field(..., description='The branch to use.\n')
...
class ConfirmPermissionsBroadeningDef(BaseClass):
    stage: typing.Union[models.StageDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The CDK Stage object to check the stacks of. This should be the same Stage object you are passing to ``addStage()``.\n')
...
class StackOutputsMapDef(BaseClass):
    pipeline: typing.Union[models.pipelines.PipelineBaseDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='-')
...
class CodeBuildOptionsDef(BaseStruct):
    build_environment: typing.Union[models.aws_codebuild.BuildEnvironmentDef, dict[str, typing.Any], None] = pydantic.Field(None, description='Partial build environment, will be combined with other build environments that apply. Default: - Non-privileged build, SMALL instance, LinuxBuildImage.STANDARD_7_0\n')
    cache: typing.Optional[models.aws_codebuild.CacheDef] = pydantic.Field(None, description='Caching strategy to use. Default: - No cache\n')
...
class CodePipelineActionFactoryResultDef(BaseStruct):
    run_orders_consumed: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='How many RunOrders were consumed. If you add 1 action, return the value 1 here.\n')
...
class FileSetLocationDef(BaseStruct):
    directory: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The (relative) directory where the FileSet is found.\n')
    file_set: typing.Union[models.pipelines.FileSetDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The FileSet object.\n\n:exampleMetadata: fixture=_generated\n\nExample::\n\n    # The code below shows an example of how to instantiate this type.\n    # The values are placeholders you should change.\n    from aws_cdk import pipelines\n\n    # file_set: pipelines.FileSet\n\n    file_set_location = pipelines.FileSetLocation(\n        directory="directory",\n        file_set=file_set\n    )\n')
...
class ProduceActionOptionsDef(BaseStruct):
    action_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Name the action should get.\n')
    artifacts: typing.Union[models.pipelines.ArtifactMapDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Helper object to translate FileSets to CodePipeline Artifacts.\n')
    pipeline: typing.Union[models.pipelines.CodePipelineDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The pipeline the action is being generated for.\n')
    run_order: typing.Union[_REQUIRED_INIT_PARAM, int, float] = pydantic.Field(REQUIRED_INIT_PARAM, description='RunOrder the action should get.\n')
    stack_outputs_map: typing.Union[models.pipelines.StackOutputsMapDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Helper object to produce variables exported from stack deployments. If your step references outputs from a stack deployment, use this to map the output references to Codepipeline variable names. Note - Codepipeline variables can only be referenced in action configurations.\n')
    before_self_mutation: typing.Optional[bool] = pydantic.Field(None, description='Whether or not this action is inserted before self mutation. If it is, the action should take care to reflect some part of its own definition in the pipeline action definition, to trigger a restart after self-mutation (if necessary). Default: false\n')
    code_build_defaults: typing.Union[models.pipelines.CodeBuildOptionsDef, dict[str, typing.Any], None] = pydantic.Field(None, description='If this action factory creates a CodeBuild step, default options to inherit. Default: - No CodeBuild project defaults\n')
    fallback_artifact: typing.Optional[models.aws_codepipeline.ArtifactDef] = pydantic.Field(None, description="An input artifact that CodeBuild projects that don't actually need an input artifact can use. CodeBuild Projects MUST have an input artifact in order to be added to the Pipeline. If the Project doesn't actually care about its input (it can be anything), it can use the Artifact passed here. Default: - A fallback artifact does not exist\n")
...
class StackAssetDef(BaseStruct):
    asset_id: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Asset identifier.\n')
    asset_manifest_path: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description="Absolute asset manifest path. This needs to be made relative at a later point in time, but when this information is parsed we don't know about the root cloud assembly yet.\n")
    asset_selector: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Asset selector to pass to ``cdk-assets``.\n')
    asset_type: typing.Union[aws_cdk.pipelines.AssetType, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Type of asset to publish.\n')
    is_template: typing.Union[bool, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Does this asset represent the CloudFormation template for the stack. Default: false\n')
...
class StackDeploymentPropsDef(BaseStruct):
    absolute_template_path: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Template path on disk to cloud assembly (cdk.out).\n')
    construct_path: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Construct path for this stack.\n')
    stack_artifact_id: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Artifact ID for this stack.\n')
    stack_name: typing.Union[str, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='Name for this stack.\n')
...
class StackStepsDef(BaseStruct):
    stack: typing.Union[models.StackDef, _REQUIRED_INIT_PARAM] = pydantic.Field(REQUIRED_INIT_PARAM, description='The stack you want the steps to run in.\n')
