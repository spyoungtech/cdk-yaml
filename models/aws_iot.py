...
class CfnJobTemplateDef(BaseCfnResource):
    ...
    destination_package_versions: typing.Optional[typing.Sequence[str]] = pydantic.Field(None, description='The package version Amazon Resource Names (ARNs) that are installed on the device’s reserved named shadow ( ``$package`` ) when the job successfully completes. *Note:* Up to 25 package version ARNS are allowed. .. epigraph:: The CloudFormation ``DestinationPackageVersions`` parameter is available in the Beijing (cn-north-1) Region and other Regions where Service Package Catalog has been deployed except for the the Ningxia (cn-northest-1) Region. We will launch to Ningxia region as quickly as possible.\n')
